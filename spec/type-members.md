...
方法、事件和属性可以实现接口成员。 若要实现接口成员，成员声明将指定 `Implements` 关键字并列出一个或多个接口成员。
...
实现接口成员的方法和属性将隐式 `NotOverridable`，除非声明为 `MustOverride`、`Overridable`或重写另一个成员。 用于实现接口成员的成员 `Shared`错误。 成员的可访问性不会影响其实现接口成员的能力。
...
如果实现的接口成员在所有显式实现的接口中都不可用，因为有多个接口继承，则实现成员必须显式引用可用于该成员的基接口。 例如，如果 `I1` 和 `I2` 包含成员 `M`，并且 `I3` 从 `I1` 和 `I2`继承，则实现 `I3` 的类型将实现 `I1.M` 和 `I2.M`。 如果接口隐藏了继承的成员，则实现类型将必须实现继承成员，并且成员将隐藏它们。
...
具有可选参数列表和可选返回值的方法是 "共享" 或 "不共享"。 通过类或类的实例访问共享方法。 非共享方法（也称为实例方法）通过类的实例进行访问。 下面的示例显示了一个类 `Stack`，该类具有若干共享方法（`Clone` 和 `Flip`）以及若干实例方法（`Push`、`Pop`和 `ToString`）：
...
有两种类型的*方法：不*返回值的方法和*函数*。 如果在接口中定义了方法或具有 `MustOverride` 修饰符，则只能省略方法的主体和 `End` 构造。 如果未在函数上指定返回类型，并且使用的是严格语义，则会发生编译时错误;否则，该类型将隐式 `Object` 或该方法类型字符的类型。 方法的返回类型和参数类型的可访问域必须与方法本身的可访问域的可访问域相同或超集。

__常规方法__是既没有 `Async` 也没有 `Iterator` 修饰符。 它可以是子例程或函数。 部分[常规方法](statements.md#regular-methods)详细说明了在调用常规方法时会发生的情况。

__迭代器方法__是具有 `Iterator` 修饰符且没有 `Async` 修饰符的方法。 它必须是一个函数，并且它的返回类型必须为某些 `T``IEnumerator`、`IEnumerable`或 `IEnumerator(Of T)` 或 `IEnumerable(Of T)`，并且它必须没有 `ByRef` 参数。 节[迭代器方法](statements.md#iterator-methods)详细说明调用迭代器方法时所发生的情况。

__Async 方法__是具有 `Async` 修饰符的方法，没有 `Iterator` 修饰符。 它必须是子例程，或具有返回类型的函数 `Task` 或 `Task(Of T)` 用于某些 `T`，并且必须没有 `ByRef` 参数。 节[Async 方法](statements.md#async-methods)详细说明了在调用 Async 方法时会发生的情况。

如果某个方法不是以下三种方法之一，则会发生编译时错误。

子程序和函数声明特别适用于每个语句的开始和结束语句，都必须从逻辑行的开头开始。 此外，非`MustOverride` 子例程或函数声明的主体必须从逻辑行的开头开始。 例如：
...
外部方法声明的 library 子句指定了实现方法的外部文件的名称。 可选的 alias 子句是一个字符串，它指定外部文件中的数字序号（以 `#` 字符作为前缀）或方法的名称。 还可以指定单字符集修饰符，此修饰符控制在调用外部方法的过程中用于封送字符串的字符集。 `Unicode` 修饰符将所有字符串封送到 Unicode 值，`Ansi` 修饰符将所有字符串封送为 ANSI 值，`Auto` 修饰符 .NET Framework 根据方法名称或别名（如果已指定）封送字符串。 如果未指定修饰符，则默认值为 `Ansi`。

如果指定 `Ansi` 或 `Unicode`，则在外部文件中查找方法名称，并且不进行修改。 如果指定 `Auto`，则方法名称查找取决于平台。 如果平台被视为 ANSI （例如，Windows 95、Windows 98、Windows ME），则将查找方法名称，但不进行任何修改。 如果查找失败，则追加 `A`，并再次尝试查找。 如果平台被视为 Unicode 平台（例如，Windows NT、Windows 2000、Windows XP），则会追加一个 `W` 并查找该名称。 如果查找失败，将再次尝试查找，而不会 `W`。 例如：
...
传递给外部方法的数据类型根据 .NET Framework 数据封送处理约定进行封送处理，但有一个例外。 通过值传递的字符串变量（即 `ByVal x As String`）将封送到 OLE 自动化 BSTR 类型，对外部方法中的 BSTR 所做的更改将反映在字符串参数中。 这是因为外部方法中的类型 `String` 是可变的，而这种特殊的封送模拟该行为。 通过引用传递的字符串参数（即 `ByRef x As String`）作为指向 OLE 自动化 BSTR 类型的指针进行封送处理。 可以通过在参数上指定 `System.Runtime.InteropServices.MarshalAsAttribute` 特性来重写这些特殊行为。
...
`Overridable` 修饰符指示方法是可重写的。 `Overrides` 修饰符指示方法重写具有相同签名的基类型可重写方法。 `NotOverridable` 修饰符指示无法进一步重写可重写的方法。 `MustOverride` 修饰符指示必须在派生类中重写方法。

这些修饰符的某些组合无效：

* `Overridable` 和 `NotOverridable` 互斥，不能组合。

* `MustOverride` 隐含 `Overridable` （因此无法指定），且不能与 `NotOverridable`结合。

* `NotOverridable` 不能与 `Overridable` 或 `MustOverride` 结合，并且必须与 `Overrides`组合。

* `Overrides` 隐含 `Overridable` （因此无法指定），且不能与 `MustOverride`结合。

对于可重写方法还存在其他限制：

* `MustOverride` 方法不能包含方法体或 `End` 构造，不能重写另一方法，并且可能仅出现在 `MustInherit` 类中。

* 如果方法指定 `Overrides` 并且没有要重写的匹配基方法，则会发生编译时错误。 重写方法不能指定 `Shadows`。

* 如果重写方法的可访问域与被重写的方法的可访问性域不相等，则方法不能重写另一个方法。 一种例外情况是，一个方法重写另一个程序集中没有 `Friend` 访问的 `Protected Friend` 方法必须指定 `Protected` （不 `Protected Friend`）。

* `Private` 方法不能 `Overridable`、`NotOverridable`或 `MustOverride`，也不会重写其他方法。

* `NotInheritable` 类中的方法不能 `Overridable` 或 `MustOverride`声明。
...
在此示例中，类 `Base` 引入了一个方法 `F` 和 `Overridable` 方法 `G`。 类 `Derived` 引入了新的方法 `F`，从而将继承的 `F`进行了隐藏，同时还会重写继承的方法 `G`。 此示例产生以下输出：
...
请注意，语句 `b.G()` 调用 `Derived.G`，而不是 `Base.G`。 这是因为实例的运行时类型（`Derived`）而不是实例的编译时类型（即 `Base`）确定要调用的实际方法实现。

### <a name="shared-methods"></a>共享方法

`Shared` 修饰符表示方法是一个*共享方法*。 共享方法不会对类型的特定实例进行操作，并且可以直接从类型调用，而不是通过类型的特定实例调用。 不过，它有效地使用实例来限定共享方法。 在共享方法中引用 `Me`、`MyClass`或 `MyBase` 是无效的。 共享方法可能不 `Overridable`、`NotOverridable`或 `MustOverride`，并且它们可能不会重写方法。 标准模块和接口中定义的方法不能指定 `Shared`，因为它们已经隐式 `Shared`。

不带 `Shared` 修饰符的结构或类中声明的方法是*实例方法*。 实例方法在类型的给定实例上操作。 仅可通过类型的实例调用实例方法，并可通过 `Me` 表达式引用实例。
...
方法 `F` 显示在实例函数成员中，标识符可用于访问实例成员和共享成员。 方法 `G` 在共享函数成员中显示，通过标识符访问实例成员是错误的。 方法 `Main` 表明在成员访问表达式中，实例成员必须通过实例访问，但共享成员可以通过类型或实例进行访问。
...
参数分别由修饰符指定为 value、reference、optional 或 paramarray 参数 `ByVal`、`ByRef`、`Optional`和 `ParamArray`。 不指定 `ByRef` 或 `ByVal` 默认为 `ByVal`的参数。

参数名称的范围限定为方法的整个主体，并且始终可公开访问。 方法调用创建特定于该调用的副本，并且调用的参数列表将值或变量引用分配给新创建的参数。 由于外部方法声明和委托声明没有正文，因此参数列表中允许有重复的参数名，但不建议使用。

标识符后面可以是可以为 null 的 name 修饰符 `?` 来指示它可以为 null，并且还可以通过数组名称修饰符来指示它是一个数组。 它们可能会组合在一起，例如 "`ByVal x?() As Integer`"。 不允许使用显式数组界限;此外，如果有可以为 null 的名称修饰符，则必须存在 `As` 子句。


#### <a name="value-parameters"></a>值参数

*值参数*使用显式 `ByVal` 修饰符进行声明。 如果使用 `ByVal` 修饰符，则不能指定 `ByRef` 修饰符。 值参数与参数所属的成员的调用一起存在，并使用调用中给定的自变量的值初始化。 返回成员时，值参数将不再存在。

允许方法为值参数赋值。 此类分配只会影响 value 参数所表示的本地存储位置;它们不会影响方法调用中给定的实际参数。

当参数的值传递到方法中时，将使用值参数，并且参数的修改不会影响原始参数。 值参数引用其自己的变量，该变量与相应参数的变量不同。 通过复制相应参数的值来初始化此变量。 下面的示例演示一个具有名为 `p`的值参数的方法 `F`：
...
引用参数是使用 `ByRef` 修饰符声明的参数。 如果指定了 `ByRef` 修饰符，则不能使用 `ByVal` 修饰符。 引用参数不会创建新的存储位置。 相反，引用参数表示作为方法或构造函数调用中的自变量提供的变量。 从概念上讲，引用参数的值始终与基础变量相同。

引用参数在两种模式下操作，可以是*别名*，也可以通过*复制回复制。*

__别名.__ 如果参数充当调用方提供的参数的别名，则使用 reference 参数。 引用参数本身不定义变量，而是指对应参数的变量。 直接修改引用参数，并立即影响相应的参数。 下面的示例演示具有两个引用参数 `Swap` 的方法：
...
对于类 `Main`中 `Swap` 方法的调用，`a` 表示 `x,`，`b` 表示 `y`。 因此，调用具有交换 `x` 和 `y`的值的效果。
...
在此示例中，调用的方法 `F` `G` 为 `a` 和 `b`传递对 `s` 的引用。 因此，对于该调用，名称 `s`、`a`和 `b` 都引用相同的存储位置，三个分配都将修改实例变量 `s`。
...
如果第一次调用 `F`，则将创建一个临时变量，并将属性 `G` 的值分配给该变量，并将其传递到 `F`中。 从 `F`返回时，会将临时变量中的值分配回 `G`的属性。 在第二种情况下，将创建另一个临时变量，并将 `d` 的值分配给它并将其传递到 `F`中。 从 `F`返回时，临时变量中的值会转换回变量的类型、`Derived`并分配给 `d`。 由于传递回的值不能转换为 `Derived`，因此在运行时将引发异常。

#### <a name="optional-parameters"></a>可选参数

可选参数是使用 `Optional` 修饰符声明的。 形参列表中可选参数之后的参数也必须是可选的;如果在以下参数上指定 `Optional` 修饰符，将触发编译时错误。 某些类型可以为 null 的类型 `T?` 或不可为 null 的类型的可选参数 `T` 必须指定一个常数表达式 `e`，如果未指定参数，则该表达式将用作默认值。 如果 `e` 的计算结果为 Object 类型的 `Nothing`，则*参数类型*的默认值将用作参数的默认值。 否则，`CType(e, T)` 必须是常量表达式，并将其作为参数的默认值。
...
`ParamArray` 参数是用 `ParamArray` 修饰符声明的。 如果 `ParamArray` 修饰符存在，则必须指定 `ByVal` 修饰符，其他参数也不能使用 `ParamArray` 修饰符。 `ParamArray` 参数的类型必须为一维数组，并且它必须是参数列表中的最后一个参数。

`ParamArray` 参数表示 `ParamArray`类型的参数数目不确定。 在方法本身中，`ParamArray` 参数被视为其声明的类型并且没有特殊语义。 `ParamArray` 参数是隐式可选的，其默认值为 `ParamArray`类型的一维一维数组。

`ParamArray` 允许在方法调用中使用以下两种方法之一指定参数：

* 为 `ParamArray` 提供的参数可以是扩大到 `ParamArray` 类型的类型的单个表达式。 在这种情况下，`ParamArray` 的作用与值形参完全相同。

* 此外，调用可以为 `ParamArray`指定零个或多个参数，其中每个参数都是可隐式转换为 `ParamArray`的元素类型的类型的表达式。 在这种情况下，调用会创建一个 `ParamArray` 类型的实例，该实例的长度与参数的数量相对应，使用给定的参数值初始化数组实例的元素，并使用新创建的数组实例作为实际的参数。

除了允许在调用中使用可变数量的参数，`ParamArray` 完全等效于同一类型的值参数，如下面的示例所示。
...
`F` 的第一次调用只是将数组作为值参数传递 `a`。 第二次调用 `F` 会自动创建包含给定元素值的四元素数组，并将该数组实例作为值参数进行传递。 同样，第三次调用 `F` 会创建一个零元素数组，并将该实例作为值参数进行传递。 第二次和第三次调用完全等效于写入：
...
不能在委托或事件声明中指定 `ParamArray` 参数。

### <a name="event-handling"></a>事件处理

方法可以声明方式处理实例中的对象或共享变量引发的事件。 若要处理事件，方法声明会指定 `Handles` 关键字并列出一个或多个事件。
...
`Handles` 列表中的事件由以句点分隔的两个标识符指定：

* 第一个标识符必须是指定 `WithEvents` 修饰符或 `MyBase` 或 `MyClass` 或 `Me` 关键字的包含类型中的实例或共享变量;否则，会发生编译时错误。 此变量包含将引发此方法处理的事件的对象。

* 第二个标识符必须指定第一个标识符的类型的成员。 该成员必须是事件，并且可以共享。 如果为第一个标识符指定了共享变量，则必须共享该事件或错误结果。

如果 `AddHandler E, AddressOf M` 的语句也是有效的，则 `M` 视为事件 `E` 的有效事件处理程序。 但与 `AddHandler` 语句不同，显式事件处理程序允许使用不带参数的方法处理事件，而不管是否使用了严格语义：
...
可以使用*扩展方法*将方法添加到类型声明外部的类型中。 扩展方法是应用了 `System.Runtime.CompilerServices.ExtensionAttribute` 特性的方法。 它们只能在标准模块中声明，并且必须至少有一个参数，该参数指定方法扩展的类型。 例如，下面的扩展方法扩展了类型 `String`：
...
调用扩展方法时，对其调用的实例将传递给第一个参数。 第一个参数不能 `ParamArray``Optional` 声明。 任何类型（包括类型参数）都可以显示为扩展方法的第一个参数。 例如，下面的方法将 `Integer()`、任何实现 `System.Collections.Generic.IEnumerable(Of T)`的类型以及所有类型的类型扩展：
...
需要注意的是，扩展方法被视为属于成员的所有上下文中的类型成员，如强类型 `For Each` 模式。 例如：
...
__纪录.__ Visual Basic 通常会在实例方法调用中插入检查，如果在上调用该方法的实例 `Nothing`，将导致 `System.NullReferenceException` 出现。 对于扩展方法，没有有效的方法来插入此项检查，因此扩展方法将需要显式检查 `Nothing`。 

__纪录.__ 在将值类型作为 `ByVal` 参数传递到类型化为接口的参数时，将对该值类型进行装箱。  这意味着扩展方法的副作用将对结构的副本（而不是原始结构）进行操作。 尽管语言对扩展方法的第一个参数没有任何限制，但建议不将扩展方法用于扩展值类型，或者在扩展值类型时，将第一个参数传递 `ByRef` 以确保对原始值执行副作用。
...
在此示例中，类的分部声明 `MyForm` 声明未实现的分部方法 `ValidateControls`。 分部声明中的构造函数调用分部方法，即使文件中未提供正文也是如此。 然后，`MyForm` 的其他分部声明提供方法的实现。
...
分部方法声明必须声明为 `Private`，且必须始终是其正文中不包含任何语句的子例程。 分部方法本身不能实现接口方法，尽管提供其主体的方法可以。
...
引用类型中的所有构造函数都必须调用另一个构造函数。 如果调用是显式的，则它必须是构造函数方法体中的第一条语句。 语句可以调用类型的实例构造函数中的另一个（例如 `Me.New(...)` 或 `MyClass.New(...)`），如果不是结构，则它可以调用该类型的基类型的实例构造函数-例如，`MyBase.New(...)`。 构造函数调用自身是无效的。 如果构造函数省略对另一个构造函数的调用，`MyBase.New()` 为隐式。 如果没有无参数的基类型构造函数，则会发生编译时错误。 由于 `Me` 在调用基类构造函数之前不被视为构造，因此构造函数调用语句的参数不能隐式或显式引用 `Me`、`MyClass`或 `MyBase`。

当构造函数的第一个语句的形式为 `MyBase.New(...)`时，构造函数将隐式执行由该类型中声明的实例变量的变量初始值设定项指定的初始化。 这对应于调用直接基类型构造函数之后立即执行的一系列赋值。 此类排序可确保在执行具有对实例的访问权限的任何语句之前，通过其变量初始值设定项来初始化所有基实例变量。 例如：
...
`1` `y` 的值，因为在调用基类构造函数之后将执行变量初始值设定项。 变量初始值设定项将按照它们在类型声明中出现的文本顺序执行。

当某个类型仅声明 `Private` 构造函数时，通常不能从类型派生其他类型或创建该类型的实例;唯一的例外是类型嵌套在类型中。 `Private` 构造函数通常用于仅包含 `Shared` 成员的类型。

如果某个类型不包含任何实例构造函数声明，则会自动提供默认构造函数。 默认构造函数只调用直接基类型的无参数构造函数。 如果直接基类型没有可访问的无参数构造函数，则会发生编译时错误。 除非 `MustInherit`类型（在这种情况下 `Protected`默认构造函数），否则默认构造函数的声明访问类型将 `Public`。

__纪录.__ `MustInherit` 类型的默认构造函数的默认访问权限是 `Protected` 的，因为不能直接创建 `MustInherit` 类。 因此，`Public`默认构造函数没有任何一点。
...
发出到使用特性标记的设计器生成的类中的默认构造函数 `Microsoft.VisualBasic.CompilerServices.DesignerGeneratedAttribute` 将调用该方法 `Sub InitializeComponent()`（如果存在），然后调用基构造函数。 （__注意：__ 这使得设计器生成的文件（如 WinForms 设计器创建的文件）可以在设计器文件中省略该构造函数。 这样一来，程序员可以自行指定它。）
...
下面的示例演示一个 `Employee` 类，该类具有用于初始化共享变量的共享构造函数：
...
每个封闭式泛型类型都存在单独的共享构造函数。 由于共享构造函数只对每个关闭类型执行一次，因此，在不能通过约束在编译时检查的类型参数上强制执行运行时检查是一个方便的位置。 例如，下面的类型使用共享构造函数来强制类型参数 `Integer` 或 `Double`：
...
与此相反，以下示例生成可预测的输出。 请注意，类的 `Shared` 构造函数 `A` 永远不会执行（即使类 `B` 从该构造函数派生）：
...
还可以构造循环依赖关系，以便在其默认值状态中观察变量初始值设定项 `Shared` 变量，如下例所示：
...
为了执行 `Main` 方法，系统首先加载类 `B`。 类 `B` 的 `Shared` 构造函数将继续计算 `Y`的初始值，这会递归导致加载类 `A`，因为 `A.X` 的值被引用。 类 `A` 的 `Shared` 构造函数将继续计算 `X`的初始值，而在执行此操作时，将获取 `Y`的*默认*值，该值为零。 `A.X` 将初始化为 `1`。 然后，将 `A` 加载的过程，并返回 `Y`的初始值的计算结果，这将成为 `2`的结果。

如果已在类 `A`中找到 `Main` 方法，则示例将生成以下输出：
...
事件用于向代码通知特定事件发生。 事件声明包含标识符、委托类型或参数列表，以及可选的 `Implements` 子句。
...
除了添加到类型声明空间的成员名称外，事件声明还会隐式声明多个其他成员。 假设有一个名为 `X`的事件，以下成员将添加到声明空间：

* 如果声明的形式为方法声明，将引入名为 `XEventHandler` 的嵌套委托类。 嵌套的委托类与方法声明匹配，并且具有与事件相同的可访问性。 参数列表中的特性适用于委托类的参数。

* 类型化为委托的 `Private` 实例变量，名为 `XEvent`。

* 名为 `add_X` 和 `remove_X` 的两个方法无法调用、重写或重载。
...
可以通过以下两种方式之一来处理事件：静态或动态。 静态处理事件更简单，只需 `WithEvents` 变量和 `Handles` 子句。 在下面的示例中，类 `Form1` 静态处理对象 `Button`的事件 `Click`：
...
动态处理事件更复杂，因为必须显式连接事件，并在代码中断开与的连接。 语句 `AddHandler` 为事件添加处理程序，语句 `RemoveHandler` 删除事件的处理程序。 下一个示例显示了一个类 `Form1`，它将 `Button1_Click` 添加为 `Button1`的 `Click` 事件的事件处理程序：
...
如上一节所述，事件声明隐式定义了字段、`add_` 方法和用于跟踪事件处理程序的 `remove_` 方法。 但在某些情况下，可能需要提供自定义代码来跟踪事件处理程序。 例如，如果某个类定义了只处理少量事件的40事件，则使用哈希表而不是40字段来跟踪每个事件的处理程序可能更高效。 *自定义事件*允许显式定义 `add_X` 和 `remove_X` 方法，这将为事件处理程序启用自定义存储。

声明自定义事件的方式与指定委托类型的事件的声明方式相同，但关键字 `Custom` 必须在 `Event` 关键字之前。 自定义事件声明包含三个声明： `AddHandler` 声明、`RemoveHandler` 声明和 `RaiseEvent` 声明。 所有声明都不能具有任何修饰符，尽管它们可以具有属性。
...
`AddHandler` 和 `RemoveHandler` 声明采用一个 `ByVal` 参数，该参数必须是该事件的委托类型。 当执行 `AddHandler` 或 `RemoveHandler` 语句（或 `Handles` 子句自动处理事件）时，将调用相应的声明。 `RaiseEvent` 声明采用与事件委托相同的参数，并将在执行 `RaiseEvent` 语句时调用。 所有声明都必须提供并被视为子例程。

请注意，`AddHandler`、`RemoveHandler` 和 `RaiseEvent` 声明在子例程具有的行位置上具有相同的限制。 开始语句、结束语句和块必须全部显示在逻辑行的开头。

除了添加到类型声明空间的成员名称外，自定义事件声明还会隐式声明多个其他成员。 假设有一个名为 `X`的事件，以下成员将添加到声明空间：
...
从 Microsoft Visual Basic 11.0 开始，在使用 `/target:winmdobj`编译的文件中声明的事件，或在此类文件中的接口中声明的事件，并在其他地方实现了不同的处理。

* 用于构建 winmd 的外部工具通常只允许某些委托类型，例如 `System.EventHandler(Of T)` 或 `System.TypedEventHandle(Of T, U)`，而不会禁止其他委托类型。

* `XEvent` 字段具有类型 `System.Runtime.InteropServices.WindowsRuntime.EventRegistrationTokenTable(Of T)`，其中 `T` 为委托类型。

* AddHandler 访问器返回 `System.Runtime.InteropServices.WindowsRuntime.EventRegistrationToken`，而 RemoveHandler 访问器采用同一类型的单个参数。
...
常量将被隐式共享。 如果声明中包含一个 `As` 子句，子句将指定声明引入的成员类型。 如果省略该类型，则将推断常量的类型。 常数的类型只能是基元类型或 `Object`。 如果常量被类型化为 `Object` 并且没有类型字符，则该常量的实类型将是常量表达式的类型。 否则，常数的类型为常数类型字符的类型。

下面的示例演示一个名为 `Constants` 的类，该类具有两个公共常数：
...
不使用 `Shared` 修饰符声明的变量称为*实例变量*。 类的每个实例都包含该类的所有实例变量的单独副本。 当创建该类型的新实例时，引用类型的实例变量就会成为存在，当没有对该实例的引用并且 `Finalize` 方法已执行时，它将不再存在。 值类型的实例变量与它所属的变量具有完全相同的生存期。 换言之，当某个值类型的变量变为存在或不存在时，将执行值类型的实例变量。
...
下面的示例演示一个 `Color` 类，该类具有名为 `redPart`、`greenPart`和 `bluePart`的内部实例变量：
...
下面是第一个此类应用程序的示例，在此示例中，颜色共享变量 `ReadOnly` 声明，以防止其他程序更改它们：
...
命名空间 `Program1` 和 `Program2` 表示单独编译的两个程序。 由于变量 `Program1.Utils.X` 声明为 `Shared ReadOnly`，因此，`Console.WriteLine` 语句输出的值在编译时是未知的，而是在运行时获取。 因此，如果更改了 `X` 的值并重新编译了 `Program1`，则 `Console.WriteLine` 语句将输出新值，即使未重新编译 `Program2` 也是如此。 但是，如果 `X` 是常量，则 `X` 的值将在编译 `Program2` 时获得，并将不受 `Program1` 中的更改的影响，直到重新编译 `Program2` 为止。

### <a name="withevents-variables"></a>WithEvents 变量

类型可以声明它处理由其实例或共享变量之一引发的一组事件，方法是声明用 `WithEvents` 修饰符引发事件的实例或共享变量。 例如：
...
在此示例中，方法 `E1Handler` 处理 `Raiser` 存储在实例变量 `x`中的类型实例引发的事件 `E1`。

`WithEvents` 修饰符导致使用前导下划线重命名变量，并将其替换为与事件挂钩同名的属性。 例如，如果变量的名称为 `F`，则将其重命名为 `_F` 并隐式声明 `F` 属性。 如果变量的新名称和其他声明之间存在冲突，则将报告编译时错误。 应用到变量的任何属性都将传送到已重命名的变量。

`WithEvents` 声明创建的隐式属性负责挂钩和解除相关的事件处理程序。 将值分配给变量时，属性首先为变量中当前实例上的事件调用 `remove` 方法（如果有），则为解除。 接下来，将进行分配，并且属性将为变量中的新实例上的事件调用 `add` 方法（挂钩新的事件处理程序）。 下面的代码等效于 "标准" 模块 `Test`的代码：
...
如果变量被类型化为结构，则将实例或共享变量声明为 `WithEvents` 是无效的。 此外，不能在结构中指定 `WithEvents`，也不能组合 `WithEvents` 和 `ReadOnly`。

### <a name="variable-initializers"></a>变量初始值设定项

结构中的类和实例变量声明（而不是共享变量声明）中的实例和共享变量声明可能包含变量初始值设定项。 对于 `Shared` 变量，变量初始值设定项对应于在程序开始后、第一次引用 `Shared` 变量之前执行的赋值语句。 对于实例变量，变量初始值设定项对应于创建类的实例时执行的赋值语句。 结构不能具有实例变量初始值设定项，因为无法修改其无参数构造函数。
...
在类加载时，将对 `x` 进行分配，并在创建类的新实例时，将分配给 `i` 和 `s`。
...
由于在类加载时 `b` 会自动初始化为其默认值，并且当创建类的实例时，`i` 会自动初始化为其默认值，以上代码将产生以下输出：
...
声明一个局部变量 `x` 其类型为一维数组的一维 `Integer`数组，该数组由第一个维度中的 `0..5` 边界 `0..10` 和第二个维度中的边界初始化为数组。 不能使用数组大小初始值设定项来初始化其类型为数组的数组的变量元素。
...
派生自类的类 `System.MarshalByRefObject` 使用代理（即按引用）在上下文边界之间进行封送处理，而不是通过复制（即通过值）进行封送处理。 这意味着此类类的实例可能不是真正的实例，而可能只是在上下文边界封送变量访问和方法调用的存根。

因此，不能创建对此类类中定义的变量的存储位置的引用。 这意味着，类型化为 `System.MarshalByRefObject` 派生的类的变量不能传递到引用参数，因此，可能无法访问类型化为值类型的变量的方法和变量。 相反，Visual Basic 将此类类上定义的变量视为属性（因为这些限制在属性上是相同的）。

此规则有一个例外：使用 `Me` 隐式或显式限定的成员不受上述限制，因为 `Me` 始终保证是实际的对象，而不是代理。

## <a name="properties"></a>属性

*属性*是变量的自然扩展;两者都是具有关联类型的命名成员，并且用于访问变量和属性的语法相同。 但与变量不同，属性不表示存储位置。 相反，属性具有*访问器*，它指定要执行的语句，以便读取或写入其值。

属性通过属性声明进行定义。 属性声明的第一部分类似于字段声明。 第二部分包括 `Get` 访问器和/或 `Set` 访问器。
...
根据上述 `Button` 类，以下是使用 `Caption` 属性的示例：
...
如果没有为属性指定类型，并且使用的是严格语义，则会发生编译时错误;否则，属性的类型将隐式 `Object` 或属性类型字符的类型。 属性声明可以包含 `Get` 访问器，该访问器检索属性的值、`Set` 访问器（存储属性的值）或同时包含两者。 由于属性隐式声明方法，因此可以使用与方法相同的修饰符来声明属性。 如果在接口中定义该属性或使用 `MustOverride` 修饰符定义该属性，则必须省略属性体和 `End` 构造;否则，会发生编译时错误。

索引参数列表构成属性的签名，因此属性可能会在索引参数上重载，而不能在属性的类型上重载。 索引参数列表与常规方法相同。 但是，不能用 `ByRef` 修饰符修改任何参数，它们都不能命名为 `Value` （在 `Set` 访问器中为隐式值参数保留）。

可按如下所示声明属性：

* 如果属性未指定属性类型修饰符，则该属性必须同时具有 `Get` 访问器和 `Set` 访问器。 此属性被称为读写属性。

* 如果属性指定 `ReadOnly` 修饰符，则该属性必须具有 `Get` 访问器且不能具有 `Set` 访问器。 此属性被称为只读属性。 只读属性是赋值的目标时，这是一个编译时错误。

* 如果属性指定 `WriteOnly` 修饰符，则该属性必须具有 `Set` 访问器且不能具有 `Get` 访问器。 此属性被称为只写属性。 在表达式中引用只写属性（作为赋值的目标或作为方法的参数）时，会发生编译时错误。

属性的 `Get` 和 `Set` 访问器不是不同的成员，并且不能单独声明属性的访问器。 下面的示例未声明单个读写属性。 相反，它声明了两个具有相同名称的属性，一个为只读，一个只写：
...
默认情况下，属性的 `Get` 和 `Set` 访问器的可访问性与属性本身的可访问性相同。 但是，`Get` 和 `Set` 访问器还可以从属性单独指定可访问性。 在这种情况下，访问器的可访问性必须比属性的可访问性更严格，而且只有一个访问器可以具有与属性不同的可访问性级别。 访问类型被视为更多或更少的限制，如下所示：

* `Private` 比 `Public`、`Protected Friend`、`Protected`或 `Friend`的限制更强。

* `Friend` 比 `Protected Friend` 或 `Public`的限制更强。

* `Protected` 比 `Protected Friend` 或 `Public`的限制更强。

* `Protected Friend` 比 `Public`更严格。
...
当派生类型隐藏属性时，派生属性将隐藏与读取和写入相关的隐藏属性。 在下面的示例中，`B` 中的 `P` 属性隐藏了 `A` 中的 `P` 属性，与读取和写入有关：
...
返回类型或参数类型的可访问域必须与属性本身的可访问域的可访问域相同或超集。 一个属性只能有一个 `Set` 访问器和一个 `Get` 访问器。

除了声明和调用语法之间的差异外，`Overridable`、`NotOverridable`、`Overrides`、`MustOverride`和 `MustInherit` 属性的行为与 `Overridable`、`NotOverridable`、`Overrides`、`MustOverride`和 `MustInherit` 方法完全相同。 当重写属性时，重写属性的类型必须相同（读写、只读、只写）。 `Overridable` 属性不能包含 `Private` 访问器。

在下面的示例中 `X` 是一个 `Overridable` 只读属性，`Y` 是 `Overridable` 读写属性，`Z` 是一个读写属性。`MustOverride`
...
由于 `Z` `MustOverride`，因此必须 `MustInherit`将包含类 `A` 进行声明。
...
此处，`X`、`Y`和 `Z` 属性的声明会重写基属性。 每个属性声明都与相应的继承属性的可访问性修饰符、类型和名称完全匹配。 属性 `X` 的 `Get` 访问器和属性 `Y` 的 `Set` 访问器使用 `MyBase` 关键字访问继承的属性。 属性的声明 `Z` 重写 `MustOverride` 属性-因此，在类 `B`中没有未完成的 `MustOverride` 成员，并且 `B` 允许成为一个常规类。
...
`ConsoleStreams` 类包含三个属性： `In`、`Out`和 `Error`，分别表示标准输入、输出和错误设备。 通过将这些成员作为属性公开，`ConsoleStreams` 类可以延迟其初始化，直到实际使用它们。 例如，在第一次引用 `Out` 属性时（如 `ConsoleStreams.Out.WriteLine("hello, world")`中），将初始化输出设备的基本 `TextWriter`。 但如果应用程序不引用 `In` 和 `Error` 属性，则不会为这些设备创建任何对象。


### <a name="get-accessor-declarations"></a>获取访问器声明

`Get` 访问器（getter）是使用属性 `Get` 声明来声明的。 属性 `Get` 声明由关键字 `Get` 后跟语句块组成。 给定一个名为 `P`的属性，`Get` 访问器声明隐式声明一个方法，其名称 `get_P` 具有与属性相同的修饰符、类型和参数列表。 如果该类型包含具有该名称的声明，则会生成编译时错误，但会忽略隐式声明以便进行名称绑定。
...
使用括号可能导致不明确的情况（例如 `F(1)`，其中 `F` 是类型为一维数组的属性）。 在所有不明确的情况下，名称解析为属性而不是局部变量。 例如：
...
当控制流离开 `Get` 访问器主体时，本地变量的值将传递回调用表达式。 由于调用 `Get` 访问器在概念上等同于读取变量的值，因此，`Get` 访问器的编程样式被视为不正确的编程样式，如以下示例中所示：
...
`NextValue` 属性的值取决于属性以前被访问的次数。 因此，访问属性会产生一个可观察到的副作用，而该属性应改为作为方法实现。

`Get` 访问器的 "无副作用" 约定并不意味着应始终将 `Get` 访问器编写为仅返回变量中存储的值。 事实上，`Get` 访问器通常通过访问多个变量或调用方法来计算属性的值。 但是，正确设计的 `Get` 访问器不会执行任何操作，从而导致对象的状态发生变化。
...
`Set` 访问器（setter）是通过使用属性集声明来声明的。 属性集声明由关键字 `Set`、可选参数列表和语句块组成。 给定一个名为 `P`的属性，setter 声明隐式声明一个名称 `set_P` 的方法，该方法具有与属性相同的修饰符和参数列表。 如果该类型包含具有该名称的声明，则会生成编译时错误，但会忽略隐式声明以便进行名称绑定。

如果指定了参数列表，则它必须具有一个成员，该成员不能除 `ByVal`以外的任何修饰符，并且其类型必须与属性的类型相同。 参数表示要设置的属性值。 如果省略该参数，则将隐式声明名为 `Value` 的参数。
...
一个属性，该属性指定 `Default` 称为*默认属性*的修饰符。 允许属性的任何类型都可以具有默认属性，包括接口。 可以引用默认属性，而无需用属性的名称限定该实例。 因此，给定一个类
...
`Default`声明属性后，在继承层次结构中该名称上重载的所有属性都将成为默认属性，无论它们是否已 `Default` 声明。 当基类通过其他名称声明默认属性时，声明派生类中 `Default` 的属性不需要任何其他修饰符，如 `Shadows` 或 `Overrides`。 这是因为默认属性没有标识或签名，因此不能被隐藏或重载。 例如：
...
在类型中声明的所有默认属性都必须具有相同的名称，为清楚起见，必须指定 `Default` 修饰符。 由于没有索引参数的默认属性会导致在分配包含类的实例时出现不明确的情况，因此默认属性必须具有索引参数。 此外，如果某个特定名称重载的一个属性包含 `Default` 修饰符，则该名称上重载的所有属性都必须指定它。 不能 `Shared`默认属性，且至少必须 `Private`属性的一个取值函数。

### <a name="automatically-implemented-properties"></a>自动实现的属性

如果属性省略了任何访问器的声明，则将自动提供属性的实现，除非该属性是在接口中声明的，或者是 `MustOverride`声明的。 只有没有参数的读/写属性才能自动实现;否则，会发生编译时错误。

自动实现的属性 `x`（甚至是另一个属性）会引入与属性具有相同类型的专用局部变量 `_x`。 如果局部变量名称和其他声明之间存在冲突，则将报告编译时错误。 自动实现的属性的 `Get` 访问器返回设置本地值的本地和属性的 `Set` 访问器的值。 例如，声明：
...
*迭代器属性*是具有 `Iterator` 修饰符的属性。 使用迭代器方法（部分[迭代器方法](statements.md#iterator-methods)）的相同原因是使用它作为生成序列的一种简便方法，该方法可由 `For Each` 语句使用。 迭代器属性的 `Get` 访问器的解释方式与迭代器方法相同。

迭代器属性必须具有显式 `Get` 访问器，并且它的类型必须为某些 `IEnumerable(Of T)` `IEnumerator`、`IEnumerable`或 `IEnumerator(Of T)` 或 `T`。
...
运算符声明不会将名称显式添加到包含类型的声明空间;但是，它们会隐式声明一个以字符 "op_" 开头的相应方法。 以下部分列出了每个运算符对应的方法名称。

可以定义三类运算符：一元运算符、二元运算符和转换运算符。 所有运算符声明都具有某些限制：

* 运算符声明必须始终 `Public` 和 `Shared`。 可以在将采用修饰符的上下文中省略 `Public` 修饰符。

* 不能 `ByRef`、`Optional` 或 `ParamArray`声明运算符的参数。

* 至少一个操作数或返回值的类型必须为包含运算符的类型。

* 没有为运算符定义函数返回变量。 因此，必须使用 `Return` 语句从运算符体返回值。

这些限制的唯一例外情况适用于可为 null 的值类型。 由于可以为 null 的值类型没有实际类型定义，因此值类型可以为类型的可以为 null 的版本声明用户定义的运算符。 在确定某一类型是否可以声明特定的用户定义运算符时，将从声明中涉及的所有类型中首次删除 `?` 修饰符，目的是进行有效性检查。 此 relaxation 不适用于 `IsTrue` 和 `IsFalse` 运算符的返回类型;它们仍必须返回 `Boolean`，而不是 `Boolean?`。
...
* 一元加运算符 `+` （对应方法： `op_UnaryPlus`）

* 一元减号运算符 `-` （对应方法： `op_UnaryNegation`）

* 逻辑 `Not` 运算符（对应的方法： `op_OnesComplement`）

* `IsTrue` 和 `IsFalse` 运算符（相应方法： `op_True`、`op_False`）

所有重载的一元运算符都必须采用包含类型的单个参数，并可能返回任何类型，`IsTrue` 和 `IsFalse`除外，必须返回 `Boolean`。 如果包含类型是泛型类型，则类型参数必须与包含类型的类型参数匹配。 例如，应用于对象的
...
如果某个类型重载 `IsTrue` 或 `IsFalse`之一，则它也必须重载另一个。 如果只有一个重载，则会产生编译时错误。

__纪录.__ `IsTrue` 和 `IsFalse` 不是保留字。

### <a name="binary-operators"></a>二元运算符

可以重载以下二进制运算符：

* 加法 `+`，减法 `-`，乘法 `*`，除法 `/`，整数除法 `\`，取模 `Mod` 和求幂 `^` 运算符（相应方法： `op_Addition`、`op_Subtraction`、`op_Multiply`、`op_Division`、`op_IntegerDivision`、`op_Modulus`、`op_Exponent`）

* 关系运算符 `=`、`<>`、`<`、`>`、`<=`、`>=` （相应方法： `op_Equality`、`op_Inequality`、`op_LessThan`、`op_GreaterThan`、`op_LessThanOrEqual`、`op_GreaterThanOrEqual`）。 __纪录.__ 尽管相等运算符可重载，但赋值运算符（仅用于赋值语句）无法重载。

* `Like` 运算符（对应方法： `op_Like`）

* 串联运算符 `&` （对应方法： `op_Concatenate`）

* 逻辑 `And`、`Or` 和 `Xor` 运算符（相应的方法： `op_BitwiseAnd`、`op_BitwiseOr`、`op_ExclusiveOr`）

* 移位运算符 `<<` 和 `>>` （相应方法： `op_LeftShift`、`op_RightShift`）

所有重载的二元运算符都必须采用包含类型作为参数之一。 如果包含类型是泛型类型，则类型参数必须与包含类型的类型参数匹配。 移位运算符会进一步限制此规则，要求第一个参数为包含类型;第二个参数必须始终为 `Integer`类型。
...
与关系运算符不同，强烈不建议同时重载除法运算符和整数除法运算符，尽管不是错误。 （__注意：__ 通常，两种类型的除法应完全不同：支持除法运算的类型是整型（在这种情况下，它应支持 `\`）或不是（在这种情况下，它应支持 `/`）。 我们认为定义这两个运算符是错误的，但由于其语言通常不能区分 Visual Basic 的两种划分方式，因此，我们认为这种做法是最安全的方法，但这种做法非常反对。）
...
转换运算符定义类型之间的新转换。 这些新转换称为*用户定义的转换*。 转换运算符从转换运算符的参数类型指示的源类型转换为目标类型，由转换运算符的返回类型指示。 转换必须分类为扩大或收缩。 包含 `Widening` 关键字的转换运算符声明引入了用户定义的扩大转换（对应的方法： `op_Implicit`）。 包含 `Narrowing` 关键字的转换运算符声明引入了用户定义的收缩转换（对应的方法： `op_Explicit`）。
...
从 `Digit` 到 `Byte` 的转换是一种扩大转换，因为它从不引发异常或丢失信息，而从 `Byte` 到 `Digit` 的转换是收缩转换，因为 `Digit` 只能表示 `Byte`的可能值的子集。
...
用户定义的转换运算符必须转换为包含类型或从包含类型转换（例如，类 `C` 可以定义从 `C` 到 `Integer` 之间的转换，以及从 `Integer` 到 `C`的转换，但不能定义从 `Integer` 到 `Boolean`的转换。 如果包含类型是泛型类型，则类型参数必须与包含类型的类型参数匹配。 此外，不能重新定义内部函数（即非用户定义的）转换。 因此，类型不能声明转换，其中：
...
这些规则的唯一例外情况适用于可为 null 的值类型。 由于可以为 null 的值类型没有实际类型定义，因此值类型可以为类型的可以为 null 的版本声明用户定义的转换。 在确定类型是否可以声明特定的用户定义的转换时，将从声明中涉及的所有类型中首次删除 `?` 修饰符，目的是进行有效性检查。 因此，以下声明有效，因为 `S` 可以定义从 `S` 到 `T`的转换：
...
不过，下面的声明无效，因为结构 `S` 不能定义从 `S` 到 `S`的转换：
...
* 重载 `Not`、`And`和 `Or` 运算符将仅从区分逻辑与按位运算符的其他语言的角度重载位运算符。

* 一个类，它仅重载一种语言中的逻辑运算符，该运算符区分逻辑运算符和位运算符（即，分别使用 `op_LogicalNot`、`op_LogicalAnd`和 `op_LogicalOr` `Not`、`And`和 `Or`的语言）将其逻辑运算符映射到 Visual Basic 逻辑运算符。 如果同时重载逻辑运算符和按位运算符，则只使用位运算符。

* 重载 `<<` 和 `>>` 运算符将仅从区分有符号和无符号移位运算符的其他语言的角度重载签名运算符。
