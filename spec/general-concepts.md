...
<span data-ttu-id="c4e35-126">重载类型成员必须具有唯一的签名。</span><span class="sxs-lookup"><span data-stu-id="c4e35-126">Overloaded type members must possess unique signatures.</span></span> <span data-ttu-id="c4e35-127">类型成员的签名包含类型参数的数目以及该成员的参数的数量和类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-127">The signature of a type member consists of the number of type parameters, and the number and types of the member's parameters.</span></span> <span data-ttu-id="c4e35-128">转换运算符还包括签名中运算符的返回类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-128">Conversion operators also include the return type of the operator in the signature.</span></span>
...
<span data-ttu-id="c4e35-154">下面是继承的重要方面：</span><span class="sxs-lookup"><span data-stu-id="c4e35-154">The following are important aspects of inheritance:</span></span>
...
<span data-ttu-id="c4e35-214">即使 @no__t @no__t 具有与 @no__t 相同的签名的展开形式，打印。</span><span class="sxs-lookup"><span data-stu-id="c4e35-214">prints `Base`, even though `Derived.F` has an expanded form that has the same signature as `Base.F`.</span></span>
...
<span data-ttu-id="c4e35-289">为 `MyBase.F()` 调用报告了错误，因为它引用了 @no__t 1 方法。</span><span class="sxs-lookup"><span data-stu-id="c4e35-289">An error is reported for the `MyBase.F()` invocation because it references a `MustOverride` method.</span></span>
...
<span data-ttu-id="c4e35-296">在此示例中，类 @no__t 提供了两个 @no__t 方法：一个方法 `F`，该方法具有 @no__t 3 修饰符，而方法 @no__t 不为-4。</span><span class="sxs-lookup"><span data-stu-id="c4e35-296">In the example, class `B` provides two `Overrides` methods: a method `F` that has the `NotOverridable` modifier and a method `G` that does not.</span></span> <span data-ttu-id="c4e35-297">使用 `NotOverridable` 修饰符可防止类 `C` 从进一步重写方法 `F`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-297">Use of the `NotOverridable` modifier prevents class `C` from further overriding method `F`.</span></span>
...
<span data-ttu-id="c4e35-324">在下面的示例中，类 @no__t 的方法 `Derived` 隐藏了可重写的方法 `F` 继承自类 `Base`：</span><span class="sxs-lookup"><span data-stu-id="c4e35-324">In the following example, method `F` in class `Derived` shadows the overridable method `F` inherited from class `Base`:</span></span>
...
* <span data-ttu-id="c4e35-356">@No__t 的可访问性域（0）和 `A.X` 无限制。</span><span class="sxs-lookup"><span data-stu-id="c4e35-356">The accessibility domain of `A` and `A.X` is unlimited.</span></span>
...
<span data-ttu-id="c4e35-364">访问 `Protected` 实例成员必须通过派生类型的实例，以便不相关的类型无法访问彼此保护的其他成员。</span><span class="sxs-lookup"><span data-stu-id="c4e35-364">Access to `Protected` instance members must be through an instance of the derived type so that unrelated types cannot gain access to each other's protected members.</span></span> <span data-ttu-id="c4e35-365">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-365">For example:</span></span>
...
<span data-ttu-id="c4e35-366">在上面的示例中，如果受保护的 `Guest` 字段是使用 `Guest` 的实例限定的，则该类 @no__t 仅有权访问该字段。</span><span class="sxs-lookup"><span data-stu-id="c4e35-366">In the above example, the class `Guest` only has access to the protected `Password` field if it is qualified with an instance of `Guest`.</span></span> <span data-ttu-id="c4e35-367">这会阻止 `Guest` 只需将其强制转换为 `User` 即可访问 @no__t 2 对象的 @no__t 字段。</span><span class="sxs-lookup"><span data-stu-id="c4e35-367">This prevents `Guest` from gaining access to the `Password` field of an `Employee` object simply by casting it to `User`.</span></span>

<span data-ttu-id="c4e35-368">出于泛型类型中 @no__t 0 成员访问的目的，声明上下文包括类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-368">For the purposes of `Protected` member access in generic types, the declaration context includes type parameters.</span></span> <span data-ttu-id="c4e35-369">这意味着，具有一组类型参数的派生类型无法访问具有一组不同类型参数的派生类型的 @no__t 的成员。</span><span class="sxs-lookup"><span data-stu-id="c4e35-369">This means that a derived type with one set of type arguments does not have access to the `Protected` members of a derived type with a different set of type arguments.</span></span> <span data-ttu-id="c4e35-370">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-370">For example:</span></span>
...
<span data-ttu-id="c4e35-371">__纪录.__</span><span class="sxs-lookup"><span data-stu-id="c4e35-371">__Note.__</span></span> <span data-ttu-id="c4e35-372">无论C#提供哪些类型参数，语言（也可能还有其他语言）都允许泛型类型访问 @no__t 1 成员。</span><span class="sxs-lookup"><span data-stu-id="c4e35-372">The C# language (and possibly other languages) allows a generic type to access `Protected` members regardless of what type arguments are supplied.</span></span> <span data-ttu-id="c4e35-373">设计包含 `Protected` 成员的泛型类时，应牢记这一点。</span><span class="sxs-lookup"><span data-stu-id="c4e35-373">This should be kept in mind when designing generic classes that contain `Protected` members.</span></span>


### <a name="constituent-types"></a><span data-ttu-id="c4e35-374">构成类型</span><span class="sxs-lookup"><span data-stu-id="c4e35-374">Constituent Types</span></span>

<span data-ttu-id="c4e35-375">声明的*构成类型*是声明引用的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-375">The *constituent types* of a declaration are the types that are referenced by the declaration.</span></span> <span data-ttu-id="c4e35-376">例如，常量的类型、方法的返回类型和构造函数的参数类型均为构成类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-376">For example, the type of a constant, the return type of a method and the parameter types of a constructor are all constituent types.</span></span> <span data-ttu-id="c4e35-377">声明的构成类型的可访问域必须与声明自身的可访问域的可访问域相同或超集。</span><span class="sxs-lookup"><span data-stu-id="c4e35-377">The accessibility domain of a constituent type of a declaration must be the same as or a superset of the accessibility domain of the declaration itself.</span></span> <span data-ttu-id="c4e35-378">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-378">For example:</span></span>
...
## <a name="type-and-namespace-names"></a><span data-ttu-id="c4e35-379">类型和命名空间名称</span><span class="sxs-lookup"><span data-stu-id="c4e35-379">Type and Namespace Names</span></span>

<span data-ttu-id="c4e35-380">许多语言构造需要指定命名空间或类型;可以通过使用命名空间或类型的名称的限定形式来指定这些。</span><span class="sxs-lookup"><span data-stu-id="c4e35-380">Many language constructs require a namespace or type to be specified; these can be specified by using a qualified form of the namespace or type's name.</span></span> <span data-ttu-id="c4e35-381">*限定名*由一系列用句点分隔的标识符组成;句点右侧的标识符在由时间段左侧的标识符指定的声明空间中解析。</span><span class="sxs-lookup"><span data-stu-id="c4e35-381">A *qualified name* consists of a series of identifiers separated by periods; the identifier on the right side of a period is resolved in the declaration space specified by the identifier on the left side of the period.</span></span>

<span data-ttu-id="c4e35-382">命名空间或类型的*完全限定名称*是一个限定名称，其中包含所有包含命名空间和类型的名称。</span><span class="sxs-lookup"><span data-stu-id="c4e35-382">The *fully qualified name* of a namespace or type is a qualified name that contains the name of all containing namespaces and types.</span></span> <span data-ttu-id="c4e35-383">换言之，命名空间或类型的完全限定名称为 `N.T`，其中 `T` 是实体的名称，`N` 是包含实体的完全限定名称。</span><span class="sxs-lookup"><span data-stu-id="c4e35-383">In other words, the fully qualified name of a namespace or type is `N.T`, where `T` is the name of the entity and `N` is the fully qualified name of its containing entity.</span></span>

<span data-ttu-id="c4e35-384">下面的示例演示了多个命名空间和类型声明及其在内联注释中的完全限定名称。</span><span class="sxs-lookup"><span data-stu-id="c4e35-384">The example below shows several namespace and type declarations together with their associated fully qualified names in in-line comments.</span></span>
...
<span data-ttu-id="c4e35-385">请注意，命名空间 X. Y 已在源代码中的两个不同位置声明，但这两个分部声明只包含一个名为 X.x 的命名空间，该命名空间同时包含类 D 和类 E。</span><span class="sxs-lookup"><span data-stu-id="c4e35-385">Observe that the namespace X.Y has been declared in two different locations in the source code, but these two partial declarations constitute just a single namespace called X.Y which contains both class D and class E.</span></span>

<span data-ttu-id="c4e35-386">在某些情况下，限定名称可能以关键字 `Global` 开头。</span><span class="sxs-lookup"><span data-stu-id="c4e35-386">In some situations, a qualified name may begin with the keyword `Global`.</span></span> <span data-ttu-id="c4e35-387">关键字表示未命名的最外侧命名空间，这在声明隐藏封闭命名空间的情况下非常有用。</span><span class="sxs-lookup"><span data-stu-id="c4e35-387">The keyword represents the unnamed outermost namespace, which is useful in situations where a declaration shadows an enclosing namespace.</span></span> <span data-ttu-id="c4e35-388">@No__t 的关键字允许在这种情况下将 "转义" 到最外面的命名空间。</span><span class="sxs-lookup"><span data-stu-id="c4e35-388">The `Global` keyword allows "escaping" out to the outermost namespace in that situation.</span></span> <span data-ttu-id="c4e35-389">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-389">For example:</span></span>
...
<span data-ttu-id="c4e35-390">在上面的示例中，第一个方法调用无效，因为标识符 `System` 绑定到类 `System`，而不是命名空间 `System`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-390">In the above example, the first method call is invalid because the identifier `System` binds to the class `System`, not the namespace `System`.</span></span> <span data-ttu-id="c4e35-391">访问 `System` 命名空间的唯一方法是使用 `Global` 以转义到最外面的命名空间。</span><span class="sxs-lookup"><span data-stu-id="c4e35-391">The only way to access the `System` namespace is to use `Global` to escape out to the outermost namespace.</span></span> <span data-ttu-id="c4e35-392">不能在 @no__t 1 语句或 `Namespace` 声明中使用 `Global`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-392">`Global` cannot be used in an `Imports` statement or `Namespace` declaration.</span></span>

<span data-ttu-id="c4e35-393">由于其他语言可能会引入与语言中的关键字匹配的类型和命名空间，因此 Visual Basic 将关键字识别为限定名称的一部分（只要它们遵循句点即可）。</span><span class="sxs-lookup"><span data-stu-id="c4e35-393">Because other languages may introduce types and namespaces that match keywords in the language, Visual Basic recognizes keywords to be part of a qualified name as long as they follow a period.</span></span> <span data-ttu-id="c4e35-394">以这种方式使用的关键字被视为标识符。</span><span class="sxs-lookup"><span data-stu-id="c4e35-394">Keywords used in this way are treated as identifiers.</span></span> <span data-ttu-id="c4e35-395">例如，限定的标识符 `X.Default.Class` 是有效的限定标识符，而 `Default.Class` 则不是。</span><span class="sxs-lookup"><span data-stu-id="c4e35-395">For example, the qualified identifier `X.Default.Class` is a valid qualified identifier, while `Default.Class` is not.</span></span>

### <a name="qualified-name-resolution-for-namespaces-and-types"></a><span data-ttu-id="c4e35-396">命名空间和类型的限定名称解析</span><span class="sxs-lookup"><span data-stu-id="c4e35-396">Qualified Name Resolution for namespaces and types</span></span>

<span data-ttu-id="c4e35-397">给定格式为 `N.R(Of A)` 格式的限定命名空间或类型名称，其中 `R` 是限定名称中最右边的标识符，`A` 是可选的类型参数列表，以下步骤描述如何确定限定的命名空间或类型表示</span><span class="sxs-lookup"><span data-stu-id="c4e35-397">Given a qualified namespace or type name of the form `N.R(Of A)`, where `R` is the rightmost identifier in the qualified name and `A` is an optional type argument list, the following steps describe how to determine to which namespace or type the qualified name refers:</span></span>

1. <span data-ttu-id="c4e35-398">使用限定或非限定名称解析的规则解析 `N`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-398">Resolve `N`, using the rules for either qualified or unqualified name resolution.</span></span>

2. <span data-ttu-id="c4e35-399">如果 `N` 的解析失败或解析为类型参数，则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-399">If resolution of `N` fails, or resolves to a type parameter, a compile-time error occurs.</span></span>

3. <span data-ttu-id="c4e35-400">否则，如果 @no__t 0 与 N 中的命名空间的名称匹配，并且未提供任何类型参数，或 `R` 与 `N` 中具有与类型参数相同数量的可访问类型（如果有）匹配，则限定名称指该命名空间或类型.</span><span class="sxs-lookup"><span data-stu-id="c4e35-400">Otherwise, if `R` matches the name of a namespace in N and no type arguments were supplied, or `R` matches an accessible type in `N` with the same number of type parameters as type arguments, if any, then the qualified name refers to that namespace or type.</span></span>

4. <span data-ttu-id="c4e35-401">否则，如果 `N` 包含一个或多个标准模块，而 `R` 与具有相同数量的类型参数（如果有）的类型参数与类型参数（如果有）的名称相匹配，则限定名称指该类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-401">Otherwise, if `N` contains one or more standard modules, and `R` matches the name of an accessible type with the same number of type parameters as type arguments, if any, in exactly one standard module, then the qualified name refers to that type.</span></span> <span data-ttu-id="c4e35-402">如果 `R` 将与类型参数数目相同的可访问类型的名称（如果有）与类型参数（如果有）相匹配，则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-402">If `R` matches the name of accessible types with the same number of type parameters as type arguments, if any, in more than one standard module, a compile-time error occurs.</span></span>

5. <span data-ttu-id="c4e35-403">否则，将发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-403">Otherwise, a compile-time error occurs.</span></span>

<span data-ttu-id="c4e35-404">__纪录.__</span><span class="sxs-lookup"><span data-stu-id="c4e35-404">__Note.__</span></span> <span data-ttu-id="c4e35-405">此解析过程的含义是，类型成员在解析命名空间或类型名称时不会隐藏命名空间或类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-405">An implication of this resolution process is that type members do not shadow namespaces or types when resolving namespace or type names.</span></span>

### <a name="unqualified-name-resolution-for-namespaces-and-types"></a><span data-ttu-id="c4e35-406">命名空间和类型的非限定名称解析</span><span class="sxs-lookup"><span data-stu-id="c4e35-406">Unqualified Name Resolution for namespaces and types</span></span>

<span data-ttu-id="c4e35-407">给定非限定名称 `R(Of A)`，其中 @no__t 为可选类型参数列表，以下步骤描述如何确定非限定名称是指哪个命名空间或类型：</span><span class="sxs-lookup"><span data-stu-id="c4e35-407">Given an unqualified name `R(Of A)`, where `A` is an optional type argument list, the following steps describe how to determine to which namespace or type the unqualified name refers:</span></span>

1. <span data-ttu-id="c4e35-408">如果 R 与当前方法的类型参数的名称匹配，并且未提供任何类型参数，则非限定名称引用该类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-408">If R matches the name of a type parameter of the current method, and no type arguments were supplied, then the unqualified name refers to that type parameter.</span></span>

2.  <span data-ttu-id="c4e35-409">对于包含名称引用的每个嵌套类型，从最内层的类型开始，转到最外层：</span><span class="sxs-lookup"><span data-stu-id="c4e35-409">For each nested type containing the name reference, starting from the innermost type and going to the outermost:</span></span>
    1. <span data-ttu-id="c4e35-410">如果 `R` 与当前类型中的类型参数的名称匹配，并且未提供任何类型参数，则非限定名称引用该类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-410">If `R` matches the name of a type parameter in the current type and no type arguments were supplied, then the unqualified name refers to that type parameter.</span></span>
    2. <span data-ttu-id="c4e35-411">否则，如果 `R` 与具有相同数量的类型参数（如果有）的可访问嵌套类型的名称匹配，则非限定名称将引用该类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-411">Otherwise, if `R` matches the name of an accessible nested type with the same number of type parameters as type arguments, if any, then the unqualified name refers to that type.</span></span>

3. <span data-ttu-id="c4e35-412">对于包含名称引用的每个嵌套命名空间，从最内层命名空间开始，转到最外面的命名空间：</span><span class="sxs-lookup"><span data-stu-id="c4e35-412">For each nested namespace containing the name reference, starting from the innermost namespace and going to the outermost namespace:</span></span>
    1. <span data-ttu-id="c4e35-413">如果 `R` 与当前命名空间中的嵌套命名空间的名称匹配，并且未提供类型参数列表，则非限定名称引用该嵌套命名空间。</span><span class="sxs-lookup"><span data-stu-id="c4e35-413">If `R` matches the name of a nested namespace in the current namespace and no type argument list is supplied, then the unqualified name refers to that nested namespace.</span></span>
    2. <span data-ttu-id="c4e35-414">否则，如果 `R` 将具有相同数量的类型参数的可访问类型的名称与当前命名空间中的类型参数（如果有）相匹配，则非限定名称将引用该类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-414">Otherwise, if `R` matches the name of an accessible type with the same number of type parameters as type arguments, if any, in the current namespace, then the unqualified name refers to that type.</span></span>
    3. <span data-ttu-id="c4e35-415">否则，如果命名空间包含一个或多个可访问的标准模块，则 `R` 会将具有相同数量的类型参数的可访问嵌套类型的名称与类型参数（如果有）匹配（如果有），然后输入非限定名称引用该嵌套类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-415">Otherwise, if the namespace contains one or more accessible standard modules, and `R` matches the name of an accessible nested type with the same number of type parameters as type arguments, if any, in exactly one standard module, then the unqualified name refers to that nested type.</span></span> <span data-ttu-id="c4e35-416">如果 `R` 将具有相同数量的类型参数的可访问嵌套类型的名称与类型参数（如果有）的名称相匹配，则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-416">If `R` matches the name of accessible nested types with the same number of type parameters as type arguments, if any, in more than one standard module, a compile-time error occurs.</span></span>

4. <span data-ttu-id="c4e35-417">如果源文件有一个或多个导入别名，并且 `R` 与其中一个别名的名称匹配，则非限定名称将引用该导入别名。</span><span class="sxs-lookup"><span data-stu-id="c4e35-417">If the source file has one or more import aliases, and `R` matches the name of one of them, then the unqualified name refers to that import alias.</span></span> <span data-ttu-id="c4e35-418">如果提供了类型参数列表，则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-418">If a type argument list is supplied, a compile-time error occurs.</span></span>

5. <span data-ttu-id="c4e35-419">如果包含名称引用的源文件有一个或多个导入：</span><span class="sxs-lookup"><span data-stu-id="c4e35-419">If the source file containing the name reference has one or more imports:</span></span>
    1. <span data-ttu-id="c4e35-420">如果 `R` 将具有相同数量的类型参数的可访问类型的名称匹配为类型参数（如果有），则只导入一个导入，而非限定名称则引用该类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-420">If `R` matches the name of an accessible type with the same number of type parameters as type arguments, if any, in exactly one import, then the unqualified name refers to that type.</span></span> <span data-ttu-id="c4e35-421">如果 `R` 匹配具有与类型参数相同数量的类型参数（如果有）的可访问类型的名称（如果有），而在多个导入和所有类型中都不是相同的类型，则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-421">If `R` matches the name of an accessible type with the same number of type parameters as type arguments, if any, in more than one import and all are not the same type, a compile-time error occurs.</span></span>
    2. <span data-ttu-id="c4e35-422">否则，如果未提供任何类型参数列表，并且 `R` 与只包含一个导入的可访问类型的命名空间的名称匹配，则非限定名称将引用该命名空间。</span><span class="sxs-lookup"><span data-stu-id="c4e35-422">Otherwise, if no type argument list was supplied and `R` matches the name of a namespace with accessible types in exactly one import, then the unqualified name refers to that namespace.</span></span> <span data-ttu-id="c4e35-423">如果未提供类型参数列表，并且 `R` 与具有可访问类型的命名空间的名称相匹配，但在多个导入中，所有不是同一个命名空间，则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-423">If no type argument list was supplied and `R` matches the name of a namespace with accessible types in more than one import and all are not the same namespace, a compile-time error occurs.</span></span>
    3. <span data-ttu-id="c4e35-424">否则，如果导入包含一个或多个可访问的标准模块，`R` 会将具有相同数量的类型参数的可访问嵌套类型的名称匹配为类型参数（如果有），而不是一个标准模块，则非限定名称是指该类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-424">Otherwise, if the imports contain one or more accessible standard modules, and `R` matches the name of an accessible nested type with the same number of type parameters as type arguments, if any, in exactly one standard module, then the unqualified name refers to that type.</span></span> <span data-ttu-id="c4e35-425">如果 `R` 将具有相同数量的类型参数的可访问嵌套类型的名称与类型参数（如果有）的名称相匹配，则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-425">If `R` matches the name of accessible nested types with the same number of type parameters as type arguments, if any, in more than one standard module, a compile-time error occurs.</span></span>

6. <span data-ttu-id="c4e35-426">如果编译环境定义了一个或多个导入别名，并且 `R` 与其中一个别名的名称匹配，则非限定名称将引用该导入别名。</span><span class="sxs-lookup"><span data-stu-id="c4e35-426">If the compilation environment defines one or more import aliases, and `R` matches the name of one of them, then the unqualified name refers to that import alias.</span></span> <span data-ttu-id="c4e35-427">如果提供了类型参数列表，则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-427">If a type argument list is supplied, a compile-time error occurs.</span></span>

7. <span data-ttu-id="c4e35-428">如果编译环境定义了一个或多个导入：</span><span class="sxs-lookup"><span data-stu-id="c4e35-428">If the compilation environment defines one or more imports:</span></span>
    1. <span data-ttu-id="c4e35-429">如果 `R` 将具有相同数量的类型参数的可访问类型的名称匹配为类型参数（如果有），则只导入一个导入，而非限定名称则引用该类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-429">If `R` matches the name of an accessible type with the same number of type parameters as type arguments, if any, in exactly one import, then the unqualified name refers to that type.</span></span> <span data-ttu-id="c4e35-430">如果 `R` 将具有相同数量的类型参数的可访问类型的名称匹配为类型参数（如果有），则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-430">If `R` matches the name of an accessible type with the same number of type parameters as type arguments, if any, in more than one import, a compile-time error occurs.</span></span>
    2. <span data-ttu-id="c4e35-431">否则，如果未提供任何类型参数列表，并且 `R` 与只包含一个导入的可访问类型的命名空间的名称匹配，则非限定名称将引用该命名空间。</span><span class="sxs-lookup"><span data-stu-id="c4e35-431">Otherwise, if no type argument list was supplied and `R` matches the name of a namespace with accessible types in exactly one import, then the unqualified name refers to that namespace.</span></span> <span data-ttu-id="c4e35-432">如果未提供类型参数列表，并且 `R` 与具有可访问类型的命名空间的名称相匹配，则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-432">If no type argument list was supplied and `R` matches the name of a namespace with accessible types in more than one import, a compile-time error occurs.</span></span>
    3. <span data-ttu-id="c4e35-433">否则，如果导入包含一个或多个可访问的标准模块，`R` 会将具有相同数量的类型参数的可访问嵌套类型的名称匹配为类型参数（如果有），而不是一个标准模块，则非限定名称是指该类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-433">Otherwise, if the imports contain one or more accessible standard modules, and `R` matches the name of an accessible nested type with the same number of type parameters as type arguments, if any, in exactly one standard module, then the unqualified name refers to that type.</span></span> <span data-ttu-id="c4e35-434">如果 `R` 将具有相同数量的类型参数的可访问嵌套类型的名称与类型参数（如果有）的名称相匹配，则会发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-434">If `R` matches the name of accessible nested types with the same number of type parameters as type arguments, if any, in more than one standard module, a compile-time error occurs.</span></span>

8. <span data-ttu-id="c4e35-435">否则，将发生编译时错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-435">Otherwise, a compile-time error occurs.</span></span>

<span data-ttu-id="c4e35-436">__纪录.__</span><span class="sxs-lookup"><span data-stu-id="c4e35-436">__Note.__</span></span> <span data-ttu-id="c4e35-437">此解析过程的含义是，类型成员在解析命名空间或类型名称时不会隐藏命名空间或类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-437">An implication of this resolution process is that type members do not shadow namespaces or types when resolving namespace or type names.</span></span>

<span data-ttu-id="c4e35-438">通常，名称只能在特定命名空间中出现一次。</span><span class="sxs-lookup"><span data-stu-id="c4e35-438">Normally, a name can only occur once in a particular namespace.</span></span> <span data-ttu-id="c4e35-439">但是，由于命名空间可以在多个 .NET 程序集中进行声明，因此在两个程序集定义具有相同完全限定名称的类型时，可能会出现这种情况。</span><span class="sxs-lookup"><span data-stu-id="c4e35-439">However, because namespaces can be declared across multiple .NET assemblies, it is possible to have a situation where two assemblies define a type with the same fully qualified name.</span></span> <span data-ttu-id="c4e35-440">在这种情况下，当前源文件集中声明的类型优先于在外部 .NET 程序集中声明的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-440">In that case, a type declared in the current set of source files is preferred over a type declared in an external .NET assembly.</span></span> <span data-ttu-id="c4e35-441">否则，名称是不明确的，没有办法消除名称的歧义。</span><span class="sxs-lookup"><span data-stu-id="c4e35-441">Otherwise, the name is ambiguous and there is no way to disambiguate the name.</span></span>

## <a name="variables"></a><span data-ttu-id="c4e35-442">变量</span><span class="sxs-lookup"><span data-stu-id="c4e35-442">Variables</span></span>

<span data-ttu-id="c4e35-443">*变量*表示存储位置。</span><span class="sxs-lookup"><span data-stu-id="c4e35-443">A *variable* represents a storage location.</span></span> <span data-ttu-id="c4e35-444">每个变量都有一个类型，用于确定可以在变量中存储的值。</span><span class="sxs-lookup"><span data-stu-id="c4e35-444">Every variable has a type that determines what values can be stored in the variable.</span></span> <span data-ttu-id="c4e35-445">由于 Visual Basic 是一种类型安全的语言，因此，程序中的每个变量都具有类型，并且该语言保证变量中存储的值始终为适当的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-445">Because Visual Basic is a type-safe language, every variable in a program has a type and the language guarantees that values stored in variables are always of the appropriate type.</span></span> <span data-ttu-id="c4e35-446">变量始终初始化为其类型的默认值，然后才可以对变量进行引用。</span><span class="sxs-lookup"><span data-stu-id="c4e35-446">Variables are always initialized to the default value of their type before any reference to the variable can be made.</span></span> <span data-ttu-id="c4e35-447">不能访问未初始化的内存。</span><span class="sxs-lookup"><span data-stu-id="c4e35-447">It is not possible to access uninitialized memory.</span></span>

## <a name="generic-types-and-methods"></a><span data-ttu-id="c4e35-448">泛型类型和方法</span><span class="sxs-lookup"><span data-stu-id="c4e35-448">Generic Types and Methods</span></span>

<span data-ttu-id="c4e35-449">类型（标准模块和枚举类型除外）和方法可以声明*类型参数，类型参数*是在声明类型的实例或调用方法之前将不提供的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-449">Types (except for standard modules and enumerated types) and methods can declare *type parameters*, which are types that will not be provided until an instance of the type is declared or the method is invoked.</span></span> <span data-ttu-id="c4e35-450">具有类型参数的类型和方法也称为*泛型类型*和*泛型方法*，因为必须对类型或方法进行一般编写，而不需要具体了解将由使用类型或方法。</span><span class="sxs-lookup"><span data-stu-id="c4e35-450">Types and methods with type parameters are also known as *generic types* and *generic methods*, respectively, because the type or method must be written generically, without specific knowledge of the types that will be supplied by code that uses the type or method.</span></span>

<span data-ttu-id="c4e35-451">__纪录.__</span><span class="sxs-lookup"><span data-stu-id="c4e35-451">__Note.__</span></span> <span data-ttu-id="c4e35-452">目前，即使方法和委托可以是泛型，属性、事件和运算符本身也不能是泛型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-452">At this time, even though methods and delegates can be generic, properties, events and operators cannot be generic themselves.</span></span> <span data-ttu-id="c4e35-453">但是，它们可以使用包含类中的类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-453">They may, however, use type parameters from the containing class.</span></span>

<span data-ttu-id="c4e35-454">从泛型类型或方法的角度来看，类型参数是一个占位符类型，它将在使用类型或方法时用实际类型填充。</span><span class="sxs-lookup"><span data-stu-id="c4e35-454">From the perspective of the generic type or method, a type parameter is a placeholder type that will be filled in with an actual type when the type or method is used.</span></span> <span data-ttu-id="c4e35-455">在使用类型或方法的点处，类型参数替换类型或方法中的类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-455">Type arguments are substituted for the type parameters in the type or method at the point at which the type or method is used.</span></span> <span data-ttu-id="c4e35-456">例如，可以将泛型 stack 类实现为：</span><span class="sxs-lookup"><span data-stu-id="c4e35-456">For example, a generic stack class could be implemented as:</span></span>
...
<span data-ttu-id="c4e35-457">使用 `Stack(Of ItemType)` 类的声明必须为类型参数提供类型参数，`ItemType`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-457">Declarations that use the `Stack(Of ItemType)` class must supply a type argument for the type parameter `ItemType`.</span></span> <span data-ttu-id="c4e35-458">然后，将在类中使用 `ItemType` 的任何位置填充此类型：</span><span class="sxs-lookup"><span data-stu-id="c4e35-458">This type is then filled in wherever `ItemType` is used within the class:</span></span>
...
### <a name="type-parameters"></a><span data-ttu-id="c4e35-459">类型参数</span><span class="sxs-lookup"><span data-stu-id="c4e35-459">Type Parameters</span></span>

<span data-ttu-id="c4e35-460">类型参数可以在类型或方法声明上提供。</span><span class="sxs-lookup"><span data-stu-id="c4e35-460">Type parameters may be supplied on type or method declarations.</span></span> <span data-ttu-id="c4e35-461">每个类型参数都是一个标识符，它是为创建构造类型或方法提供的类型自变量的占位符。</span><span class="sxs-lookup"><span data-stu-id="c4e35-461">Each type parameter is an identifier which is a place-holder for a type argument that is supplied to create a constructed type or method.</span></span> <span data-ttu-id="c4e35-462">与此相反，类型参数是在使用泛型类型或方法时替换类型参数的实际类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-462">By contrast, a type argument is the actual type that is substituted for the type parameter when a generic type or method is used.</span></span>
...
<span data-ttu-id="c4e35-463">类型或方法声明中的每个类型参数在该类型或方法的声明空间中定义一个名称。</span><span class="sxs-lookup"><span data-stu-id="c4e35-463">Each type parameter in a type or method declaration defines a name in the declaration space of that type or method.</span></span> <span data-ttu-id="c4e35-464">因此，它不能与另一个类型参数、类型成员、方法参数或局部变量具有相同的名称。</span><span class="sxs-lookup"><span data-stu-id="c4e35-464">Thus, it cannot have the same name as another type parameter, a type member, a method parameter, or a local variable.</span></span> <span data-ttu-id="c4e35-465">类型或方法的类型参数的作用域是整个类型或方法。</span><span class="sxs-lookup"><span data-stu-id="c4e35-465">The scope of a type parameter on a type or method is the entire type or method.</span></span> <span data-ttu-id="c4e35-466">由于类型参数的作用域为整个类型声明，因此嵌套类型可以使用外部类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-466">Because type parameters are scoped to the entire type declaration, nested types can use outer type parameters.</span></span> <span data-ttu-id="c4e35-467">这也意味着在访问嵌套在泛型类型中的类型时，必须始终指定类型参数：</span><span class="sxs-lookup"><span data-stu-id="c4e35-467">This also means that type parameters must always be specified when accessing types nested inside generic types:</span></span>
...
<span data-ttu-id="c4e35-468">与类的其他成员不同，类型参数不能继承。</span><span class="sxs-lookup"><span data-stu-id="c4e35-468">Unlike other members of a class, type parameters are not inherited.</span></span> <span data-ttu-id="c4e35-469">类型中的类型参数只能由其简单名称引用;换句话说，不能用包含类型名称对它们进行限定。</span><span class="sxs-lookup"><span data-stu-id="c4e35-469">Type parameters in a type can only be referred to by their simple name; in other words, they cannot be qualified with the containing type name.</span></span> <span data-ttu-id="c4e35-470">尽管编程样式不正确，但嵌套类型中的类型参数可以隐藏在外部类型中声明的成员或类型参数：</span><span class="sxs-lookup"><span data-stu-id="c4e35-470">Although it is bad programming style, the type parameters in a nested type can hide a member or type parameter declared in the outer type:</span></span>
...
<span data-ttu-id="c4e35-471">类型和方法可基于类型或方法声明的类型参数（或*arity*）的数目进行重载。</span><span class="sxs-lookup"><span data-stu-id="c4e35-471">Types and methods may be overloaded based on the number of type parameters (or *arity*) that the types or methods declare.</span></span> <span data-ttu-id="c4e35-472">例如，以下声明是合法的：</span><span class="sxs-lookup"><span data-stu-id="c4e35-472">For example, the following declarations are legal:</span></span>
...
<span data-ttu-id="c4e35-473">对于类型，重载始终与指定的类型参数的数目匹配。</span><span class="sxs-lookup"><span data-stu-id="c4e35-473">In the case of types, overloads are always matched against the number of type arguments specified.</span></span> <span data-ttu-id="c4e35-474">在同一程序中同时使用泛型和非泛型类时，这很有用：</span><span class="sxs-lookup"><span data-stu-id="c4e35-474">This is useful when using both generic and non-generic classes together in the same program:</span></span>
...
<span data-ttu-id="c4e35-475">方法重载决策部分介绍了类型参数上的重载方法规则。</span><span class="sxs-lookup"><span data-stu-id="c4e35-475">Rules for methods overloaded on type parameters are covered in the section on method overload resolution.</span></span>

<span data-ttu-id="c4e35-476">在包含声明中，类型参数被视为完全类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-476">Within the containing declaration, type parameters are considered full types.</span></span> <span data-ttu-id="c4e35-477">由于类型参数可以使用许多不同的实际类型参数进行实例化，因此类型参数与其他类型的操作和限制略有不同，如下所述：</span><span class="sxs-lookup"><span data-stu-id="c4e35-477">Since a type parameter can be instantiated with many different actual type arguments, type parameters have slightly different operations and restrictions than other types as described below:</span></span>

* <span data-ttu-id="c4e35-478">类型参数不能直接用于声明基类或接口。</span><span class="sxs-lookup"><span data-stu-id="c4e35-478">A type parameter cannot be used directly to declare a base class or interface.</span></span>

* <span data-ttu-id="c4e35-479">对类型参数进行成员查找的规则取决于应用于该类型参数的约束（如果有）。</span><span class="sxs-lookup"><span data-stu-id="c4e35-479">The rules for member lookup on type parameters depend on the constraints, if any, applied to the type parameter.</span></span>

* <span data-ttu-id="c4e35-480">类型参数的可用转换取决于应用于类型参数的约束（如果有）。</span><span class="sxs-lookup"><span data-stu-id="c4e35-480">The available conversions for a type parameter depend on the constraints, if any, applied to the type parameters.</span></span>

* <span data-ttu-id="c4e35-481">如果没有 `Structure` 约束，则可以使用 `Is` 和 `IsNot` 将类型参数表示的值与 `Nothing` 进行比较。</span><span class="sxs-lookup"><span data-stu-id="c4e35-481">In the absence of a `Structure` constraint, a value with a type represented by a type parameter can be compared with `Nothing` using `Is` and `IsNot`.</span></span>

* <span data-ttu-id="c4e35-482">如果类型参数受 `New` 或 `Structure` 约束的约束，则类型参数仅可用于 @no__t 0 表达式。</span><span class="sxs-lookup"><span data-stu-id="c4e35-482">A type parameter can only be used in a `New` expression if the type parameter is constrained by a `New` or a `Structure` constraint.</span></span>

* <span data-ttu-id="c4e35-483">类型参数不能在 @no__t 0 表达式内的属性异常中的任何位置使用。</span><span class="sxs-lookup"><span data-stu-id="c4e35-483">A type parameter cannot be used anywhere within an attribute exception within a `GetType` expression.</span></span>

* <span data-ttu-id="c4e35-484">类型参数可用作其他泛型类型和参数的类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-484">Type parameters can be used as type arguments to other generic types and parameters.</span></span>

<span data-ttu-id="c4e35-485">下面的示例是一个扩展 `Stack(Of ItemType)` 类的泛型类型：</span><span class="sxs-lookup"><span data-stu-id="c4e35-485">The following example is a generic type that extends the `Stack(Of ItemType)` class:</span></span>
...
<span data-ttu-id="c4e35-486">当声明向 @no__t 提供类型参数时，相同的类型参数也将应用于 `Stack`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-486">When a declaration supplies a type argument to `MyStack`, the same type argument will be applied to `Stack` as well.</span></span>

<span data-ttu-id="c4e35-487">类型参数只是一种编译时构造。</span><span class="sxs-lookup"><span data-stu-id="c4e35-487">As a type, type parameters are purely a compile-time construct.</span></span> <span data-ttu-id="c4e35-488">在运行时，每个类型参数都绑定到一个运行时类型，该类型是通过向泛型声明提供类型参数来指定的。</span><span class="sxs-lookup"><span data-stu-id="c4e35-488">At run-time, each type parameter is bound to a run-time type that was specified by supplying a type argument to the generic declaration.</span></span> <span data-ttu-id="c4e35-489">因此，使用类型参数声明的变量的类型将在运行时为非泛型类型或特定构造类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-489">Thus, the type of a variable declared with a type parameter will, at run-time, be a non-generic type or a specific constructed type.</span></span> <span data-ttu-id="c4e35-490">所有涉及类型参数的语句和表达式的运行时执行都使用作为该参数的类型参数提供的实际类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-490">The run-time execution of all statements and expressions involving type parameters uses the actual type that was supplied as the type argument for that parameter.</span></span>


### <a name="type-constraints"></a><span data-ttu-id="c4e35-491">类型约束</span><span class="sxs-lookup"><span data-stu-id="c4e35-491">Type Constraints</span></span>

<span data-ttu-id="c4e35-492">因为类型参数可以是类型系统中的任何类型，所以泛型类型或方法无法对类型参数作出任何假设。</span><span class="sxs-lookup"><span data-stu-id="c4e35-492">Because a type argument can be any type in the type system, a generic type or method cannot make any assumptions about a type parameter.</span></span> <span data-ttu-id="c4e35-493">因此，类型参数的成员被视为 `Object` 类型的成员，因为所有类型都派生自 `Object`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-493">Thus, the members of a type parameter are considered to be the members of the type `Object`, since all types derive from `Object`.</span></span>

<span data-ttu-id="c4e35-494">对于 @no__t 为-0 的集合，这种情况可能不是特别重要的限制，但在某些情况下，泛型类型可能想要对将作为类型参数提供的类型做出假设。</span><span class="sxs-lookup"><span data-stu-id="c4e35-494">In the case of a collection like `Stack(Of ItemType)`, this fact may not be a particularly important restriction, but there may be cases where a generic type may wish to make an assumption about the types that will be supplied as type arguments.</span></span> <span data-ttu-id="c4e35-495">*类型约束*可放置在类型参数上，该类型参数限制可以作为类型参数提供的类型，并允许泛型类型或方法更多地使用类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-495">*Type constraints* can be placed on type parameters that restrict which types can be supplied as a type parameter and allow generic types or methods to assume more about type parameters.</span></span>
...
<span data-ttu-id="c4e35-496">在此示例中，`DisposableStack(Of ItemType)` 会将其类型参数限制为仅实现接口 @no__t 的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-496">In this example, the `DisposableStack(Of ItemType)` constrains its type parameter to only types that implement the interface `System.IDisposable`.</span></span> <span data-ttu-id="c4e35-497">因此，它可以实现 `Dispose` 方法，该方法可释放仍保留在队列中的任何对象。</span><span class="sxs-lookup"><span data-stu-id="c4e35-497">As a result, it can implement a `Dispose` method that disposes any objects still left in the queue.</span></span>

<span data-ttu-id="c4e35-498">类型约束必须是特殊约束中的一个 `Class`、`Structure` 或 `New`，或者必须是一个 `T` 类型，其中：</span><span class="sxs-lookup"><span data-stu-id="c4e35-498">A type constraint must be one of the special constraints `Class`, `Structure`, or `New`, or it must be a type `T` where:</span></span>

* <span data-ttu-id="c4e35-499">@no__t 为类、接口或类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-499">`T` is a class, an interface, or a type parameter.</span></span>

* <span data-ttu-id="c4e35-500">`T` 不是 `NotInheritable`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-500">`T` is not `NotInheritable`.</span></span>

* <span data-ttu-id="c4e35-501">`T` 既不是，也不是继承自以下特殊类型之一的类型： `System.Array`、`System.Delegate`、`System.MulticastDelegate`、`System.Enum` 或 @no__t 5。</span><span class="sxs-lookup"><span data-stu-id="c4e35-501">`T` is not one of, or a type inherited from one of, the following special types: `System.Array`, `System.Delegate`, `System.MulticastDelegate`, `System.Enum`, or `System.ValueType`.</span></span>

* <span data-ttu-id="c4e35-502">`T` 不是 `Object`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-502">`T` is not `Object`.</span></span> <span data-ttu-id="c4e35-503">由于所有类型都派生自 `Object`，因此如果允许，此类约束将不起作用。</span><span class="sxs-lookup"><span data-stu-id="c4e35-503">Since all types derive from `Object`, such a constraint would have no effect if it were permitted.</span></span>

* <span data-ttu-id="c4e35-504">`T` 必须至少与要声明的泛型类型或方法具有相同的可访问性。</span><span class="sxs-lookup"><span data-stu-id="c4e35-504">`T` must be at least as accessible as the generic type or method being declared.</span></span>

<span data-ttu-id="c4e35-505">可以通过将类型约束括在大括号中（`{}`），为单个类型形参指定多个类型约束。</span><span class="sxs-lookup"><span data-stu-id="c4e35-505">Multiple type constraints can be specified for a single type parameter by enclosing the type constraints in curly braces (`{}`)..</span></span> <span data-ttu-id="c4e35-506">给定类型参数仅有一个类型约束可以为类。</span><span class="sxs-lookup"><span data-stu-id="c4e35-506">Only one type constraint for a given type parameter can be a class.</span></span> <span data-ttu-id="c4e35-507">将 @no__t 0 的特殊约束与命名类约束或 `Class` 特殊约束合并是错误的。</span><span class="sxs-lookup"><span data-stu-id="c4e35-507">It is an error to combine a `Structure` special constraint with a named class constraint or the `Class` special constraint.</span></span>
...
<span data-ttu-id="c4e35-508">类型约束可以使用包含类型或任何包含类型的类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-508">Type constraints can use the containing types or any of the containing types' type parameters.</span></span> <span data-ttu-id="c4e35-509">在下面的示例中，约束要求提供的类型参数实现泛型接口，并使用其自身作为类型参数：</span><span class="sxs-lookup"><span data-stu-id="c4e35-509">In the following example, the constraint requires that the type argument supplied implements a generic interface using itself as a type argument:</span></span>
...
<span data-ttu-id="c4e35-510">特殊类型约束 `Class` 将所提供的类型参数限制为任何引用类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-510">The special type constraint `Class` constrains the supplied type argument to any reference type.</span></span>

<span data-ttu-id="c4e35-511">__纪录.__</span><span class="sxs-lookup"><span data-stu-id="c4e35-511">__Note.__</span></span> <span data-ttu-id="c4e35-512">接口可以满足特殊类型约束 `Class`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-512">The special type constraint `Class` can be satisfied by an interface.</span></span> <span data-ttu-id="c4e35-513">结构可以实现接口。</span><span class="sxs-lookup"><span data-stu-id="c4e35-513">And a structure can implement an interface.</span></span> <span data-ttu-id="c4e35-514">因此，在 "T" 结构（不满足第 1 @no__t 特殊约束）和 "U" 实现的接口（这满足了 `Class` 特殊约束）的情况下，可能满足约束 `(Of T As U, U As Class)`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-514">Therefore, the constraint `(Of T As U, U As Class)` might be satisfied with "T" a structure (which does not satisfy the `Class` special constraint), and "U" an interface that it implements (which does satisfy the `Class` special constraint).</span></span>

<span data-ttu-id="c4e35-515">特殊类型约束 `Structure` 将所提供的类型参数限制为除 @no__t 以外的任何值类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-515">The special type constraint `Structure` constrains the supplied type argument to any value type except `System.Nullable(Of T)`.</span></span>

<span data-ttu-id="c4e35-516">__纪录.__</span><span class="sxs-lookup"><span data-stu-id="c4e35-516">__Note.__</span></span> <span data-ttu-id="c4e35-517">结构约束不允许 `System.Nullable(Of T)`，因此不能将 @no__t 1 作为其自身的类型参数提供。</span><span class="sxs-lookup"><span data-stu-id="c4e35-517">Structure constraints do not allow `System.Nullable(Of T)` so that it is not possible to supply `System.Nullable(Of T)` as a type argument to itself.</span></span>

<span data-ttu-id="c4e35-518">特殊类型约束 `New` 要求提供的类型参数必须具有可访问的无参数构造函数，并且不能 `MustInherit` 声明。</span><span class="sxs-lookup"><span data-stu-id="c4e35-518">The special type constraint `New` requires that the supplied type argument must have an accessible parameterless constructor and cannot be declared `MustInherit`.</span></span> <span data-ttu-id="c4e35-519">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-519">For example:</span></span>
...
<span data-ttu-id="c4e35-520">类类型约束要求提供的类型参数必须是该类型或从其继承。</span><span class="sxs-lookup"><span data-stu-id="c4e35-520">A class type constraint requires that the supplied type argument must either be that type as or inherit from it.</span></span> <span data-ttu-id="c4e35-521">接口类型约束要求提供的类型参数必须实现该接口。</span><span class="sxs-lookup"><span data-stu-id="c4e35-521">An interface type constraint requires that the supplied type argument must implement that interface.</span></span> <span data-ttu-id="c4e35-522">类型参数约束要求提供的类型参数必须派生自或实现为匹配的类型参数提供的所有边界。</span><span class="sxs-lookup"><span data-stu-id="c4e35-522">A type parameter constraint requires that the supplied type argument must derive from or implement all of the bounds given for the matching type parameter.</span></span> <span data-ttu-id="c4e35-523">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-523">For example:</span></span>
...
<span data-ttu-id="c4e35-524">在此示例中，`AddRange` 上 `S` 的类型参数被限制为 `List` 的类型参数 `T`。</span><span class="sxs-lookup"><span data-stu-id="c4e35-524">In this example, the type parameter `S` on `AddRange` is constrained to the type parameter `T` of `List`.</span></span> <span data-ttu-id="c4e35-525">这意味着 `List(Of Control)` 会将 @no__t 1 的类型参数限制为任何从 `Control` 或继承的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-525">This means that a `List(Of Control)` would constrain `AddRange`'s type parameter to any type that is or inherits from `Control`.</span></span>

<span data-ttu-id="c4e35-526">@No__t-0 的类型参数约束通过向除特殊约束之外的 `S` （`Class`，`Structure`，`New`）间接添加所有 @no__t 的约束来解析。</span><span class="sxs-lookup"><span data-stu-id="c4e35-526">A type parameter constraint `Of S As T` is resolved by transitively adding all of `T`'s constraints onto `S`, other than the special constraints (`Class`, `Structure`, `New`).</span></span> <span data-ttu-id="c4e35-527">具有循环约束的错误（例如 `Of S As T, T As S`）。</span><span class="sxs-lookup"><span data-stu-id="c4e35-527">It is an error to have circular constraints (e.g. `Of S As T, T As S`).</span></span> <span data-ttu-id="c4e35-528">如果类型参数约束本身具有 @no__t 的约束，则是错误的。</span><span class="sxs-lookup"><span data-stu-id="c4e35-528">It is an error to have a type parameter constraint which itself has the `Structure` constraint.</span></span> <span data-ttu-id="c4e35-529">添加约束后，可能会出现很多特殊情况：</span><span class="sxs-lookup"><span data-stu-id="c4e35-529">After adding constraints, it is possible that a number of special situations may occur:</span></span>

* <span data-ttu-id="c4e35-530">如果存在多个类约束，派生程度最高的类将被视为约束。</span><span class="sxs-lookup"><span data-stu-id="c4e35-530">If multiple class constraints exist, the most derived class is considered to be the constraint.</span></span> <span data-ttu-id="c4e35-531">如果一个或多个类约束没有继承关系，则约束为满足，这是一个错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-531">If one or more class constraints have no inheritance relationship, the constraint is unsatisfiable and it is an error.</span></span>

 * <span data-ttu-id="c4e35-532">如果类型参数将 @no__t 0 个特殊约束与命名类约束或 `Class` 特殊约束组合在一起，则为错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-532">If a type parameter combines a `Structure` special constraint with a named class constraint or the `Class` special constraint, it is an error.</span></span> <span data-ttu-id="c4e35-533">类约束可以 `NotInheritable`，在这种情况下，将不接受该约束的派生类型并且它是错误。</span><span class="sxs-lookup"><span data-stu-id="c4e35-533">A class constraint may be `NotInheritable`, in which case no derived types of that constraint are accepted and it is an error.</span></span>

<span data-ttu-id="c4e35-534">该类型可以是或从继承的类型之一，以下特殊类型： `System.Array`、`System.Delegate`、`System.MulticastDelegate`、`System.Enum` 或 @no__t。</span><span class="sxs-lookup"><span data-stu-id="c4e35-534">The type may be one of, or a type inherited from, the following special types: `System.Array`, `System.Delegate`, `System.MulticastDelegate`, `System.Enum`, or `System.ValueType`.</span></span> <span data-ttu-id="c4e35-535">在这种情况下，只接受类型或从其继承的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-535">In that case, only the type, or a type inherited from it, is accepted.</span></span> <span data-ttu-id="c4e35-536">约束为这些类型之一的类型参数只能使用 `DirectCast` 运算符允许的转换。</span><span class="sxs-lookup"><span data-stu-id="c4e35-536">A type parameter constrained to one of these types can only use the conversions allowed by the `DirectCast` operator.</span></span> <span data-ttu-id="c4e35-537">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-537">For example:</span></span>
...
<span data-ttu-id="c4e35-538">此外，由于上述某个松弛法，类型参数被限制为值类型，因此无法调用对该值类型定义的任何方法。</span><span class="sxs-lookup"><span data-stu-id="c4e35-538">Additionally, a type parameter constrained to a value type due to one of the above relaxations cannot call any methods defined on that value type.</span></span> <span data-ttu-id="c4e35-539">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-539">For example:</span></span>
...
<span data-ttu-id="c4e35-540">如果在替换后，约束结束为数组类型，则还允许任何协变数组类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-540">If the constraint, after substitution, ends up as an array type, any covariant array type is allowed as well.</span></span> <span data-ttu-id="c4e35-541">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-541">For example:</span></span>
...
<span data-ttu-id="c4e35-542">具有类或接口约束的类型参数被视为具有与该类或接口约束相同的成员。</span><span class="sxs-lookup"><span data-stu-id="c4e35-542">A type parameter with a class or interface constraint is considered to have the same members as that class or interface constraint.</span></span> <span data-ttu-id="c4e35-543">如果类型参数具有多个约束，则类型参数被视为具有约束的所有成员的并集。</span><span class="sxs-lookup"><span data-stu-id="c4e35-543">If a type parameter has multiple constraints, then the type parameter is considered to have the union of all the members of the constraints.</span></span> <span data-ttu-id="c4e35-544">如果有多个约束中具有相同名称的成员，则成员将按以下顺序隐藏：类约束隐藏了接口约束中的成员，这些成员隐藏了 `System.ValueType` 中的成员（如果指定了 @no__t 1 约束），这将隐藏@no__t 中的成员。</span><span class="sxs-lookup"><span data-stu-id="c4e35-544">If there are members with the same name in more than one constraint, then members are hidden in the following order: the class constraint hides members in interface constraints, which hide members in `System.ValueType` (if `Structure` constraint is specified), which hides members in `Object`.</span></span> <span data-ttu-id="c4e35-545">如果具有相同名称的成员出现在多个接口约束中，则成员不可用（如在多个接口继承中），并且类型参数必须强制转换为所需的接口。</span><span class="sxs-lookup"><span data-stu-id="c4e35-545">If a member with the same name appears in more than one interface constraint the member is unavailable (as in multiple interface inheritance) and the type parameter must be cast to the desired interface.</span></span> <span data-ttu-id="c4e35-546">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-546">For example:</span></span>
...
<span data-ttu-id="c4e35-547">当提供类型参数作为类型参数时，类型参数必须满足匹配类型参数的约束。</span><span class="sxs-lookup"><span data-stu-id="c4e35-547">When supplying type parameters as type arguments, the type parameters must satisfy the constraints of the matching type parameters.</span></span>
...
<span data-ttu-id="c4e35-548">受约束的类型参数的值可用于访问在约束中指定的实例成员（包括实例方法）。</span><span class="sxs-lookup"><span data-stu-id="c4e35-548">Values of a constrained type parameter can be used to access the instance members, including instance methods, specified in the constraint.</span></span>
...
### <a name="type-parameter-variance"></a><span data-ttu-id="c4e35-549">类型参数的方差</span><span class="sxs-lookup"><span data-stu-id="c4e35-549">Type Parameter Variance</span></span>

<span data-ttu-id="c4e35-550">接口或委托类型声明中的类型参数可以选择指定*方差修饰符*。</span><span class="sxs-lookup"><span data-stu-id="c4e35-550">A type parameter in an interface or a delegate type declaration can optionally specify a *variance modifier*.</span></span> <span data-ttu-id="c4e35-551">具有变体修饰符的类型参数限制了类型参数可在接口或委托类型中的使用方式，但允许泛型接口或委托类型转换为具有变体兼容类型参数的另一个泛型类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-551">Type parameters with variance modifiers restrict how the type parameter can be used in the interface or delegate type but allow a generic interface or delegate type to be converted to another generic type with variant compatible type arguments.</span></span> <span data-ttu-id="c4e35-552">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-552">For example:</span></span>
...
<span data-ttu-id="c4e35-553">具有具有变体修饰符的类型参数的泛型接口具有几个限制：</span><span class="sxs-lookup"><span data-stu-id="c4e35-553">Generic interfaces that have type parameters with variance modifiers have several restrictions:</span></span>

* <span data-ttu-id="c4e35-554">它们不能包含指定参数列表的事件声明（但允许使用委托类型的自定义事件声明或事件声明）。</span><span class="sxs-lookup"><span data-stu-id="c4e35-554">They cannot contain an event declaration that specifies a parameter list (but a custom event declaration or an event declaration with a delegate type is allowed).</span></span>

* <span data-ttu-id="c4e35-555">它们不能包含嵌套的类、结构或枚举类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-555">They cannot contain a nested class, structure, or enumerated type.</span></span>

<span data-ttu-id="c4e35-556">__纪录.__</span><span class="sxs-lookup"><span data-stu-id="c4e35-556">__Note.__</span></span> <span data-ttu-id="c4e35-557">这些限制的原因在于，嵌套在泛型类型中的类型隐式复制其父级的泛型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-557">These restrictions are due to the fact that types nested in generic types implicitly copy the generic parameters of their parent.</span></span> <span data-ttu-id="c4e35-558">对于嵌套类、结构或枚举类型，这些类型的类型不能对其类型参数具有变体修饰符。</span><span class="sxs-lookup"><span data-stu-id="c4e35-558">In the case of nested classes, structures, or enumerated types, those kinds of types cannot have variance modifiers on their type parameters.</span></span> <span data-ttu-id="c4e35-559">对于带有参数列表的事件声明，当在 `In` 位置（即参数类型）中实际使用的类型实际用于 `Out` 位置（即，的类型）时，生成的嵌套委托类可能出现混淆错误event）。</span><span class="sxs-lookup"><span data-stu-id="c4e35-559">In the case of an event declaration with a parameter list, the generated nested delegate class could have confusing errors when a type that appears to be used in an `In` position (i.e. a parameter type) is actually used in an `Out` position (i.e. the type of the event).</span></span>

<span data-ttu-id="c4e35-560">使用 Out 修饰符声明的类型参数是*协变*的。</span><span class="sxs-lookup"><span data-stu-id="c4e35-560">A type parameter that is declared with the Out modifier is *covariant*.</span></span> <span data-ttu-id="c4e35-561">非正式类型参数只能在输出位置中使用--即从接口或委托类型返回的值，而不能在输入位置使用。</span><span class="sxs-lookup"><span data-stu-id="c4e35-561">Informally, a covariant type parameter can only be used in an output position -- i.e. a value that is being returned from the interface or delegate type -- and cannot be used in an input position.</span></span> <span data-ttu-id="c4e35-562">如果以下情况，类型 `T` 被视为*有效的 covariantly* ：</span><span class="sxs-lookup"><span data-stu-id="c4e35-562">A type `T` is considered to be *valid covariantly* if:</span></span>

* <span data-ttu-id="c4e35-563">@no__t 为类、结构或枚举类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-563">`T` is a class, structure, or enumerated type.</span></span>

* <span data-ttu-id="c4e35-564">@no__t 为非泛型委托或接口类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-564">`T` is non-generic delegate or interface type.</span></span>

* <span data-ttu-id="c4e35-565">@no__t 为数组类型，其元素类型为 covariantly。</span><span class="sxs-lookup"><span data-stu-id="c4e35-565">`T` is an array type whose element type is valid covariantly.</span></span>

* <span data-ttu-id="c4e35-566">`T` 是未声明为 @no__t 类型参数的类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-566">`T` is a type parameter which was not declared as an `Out` type parameter.</span></span>

* <span data-ttu-id="c4e35-567">@no__t 为构造接口或委托类型，`X(Of P1,...,Pn)` 的类型参数 `A1,...,An`，以便：</span><span class="sxs-lookup"><span data-stu-id="c4e35-567">`T` is a constructed interface or delegate type `X(Of P1,...,Pn)` with type arguments `A1,...,An` such that:</span></span>

  * <span data-ttu-id="c4e35-568">如果 `Pi` 被声明为 Out 类型参数，则 `Ai` 是有效的 covariantly。</span><span class="sxs-lookup"><span data-stu-id="c4e35-568">If `Pi` was declared as an Out type parameter then `Ai` is valid covariantly.</span></span>

  * <span data-ttu-id="c4e35-569">如果 `Pi` 在类型参数中被声明为，则 `Ai` 是有效的 contravariantly。</span><span class="sxs-lookup"><span data-stu-id="c4e35-569">If `Pi` was declared as an In type parameter then `Ai` is valid contravariantly.</span></span>

<span data-ttu-id="c4e35-570">以下内容必须是接口或委托类型中的有效 covariantly：</span><span class="sxs-lookup"><span data-stu-id="c4e35-570">The following must be valid covariantly in an interface or delegate type:</span></span>

* <span data-ttu-id="c4e35-571">接口的基接口。</span><span class="sxs-lookup"><span data-stu-id="c4e35-571">The base interface of an interface.</span></span>

* <span data-ttu-id="c4e35-572">函数或委托类型的返回类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-572">The return type of a function or the delegate type.</span></span>

* <span data-ttu-id="c4e35-573">如果有 @no__t 0 取值函数，则为属性的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-573">The type of a property if there is a `Get` accessor.</span></span>

* <span data-ttu-id="c4e35-574">任何 @no__t 参数的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-574">The type of any `ByRef` parameter.</span></span>

<span data-ttu-id="c4e35-575">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-575">For example:</span></span>
...
<span data-ttu-id="c4e35-576">__纪录.__</span><span class="sxs-lookup"><span data-stu-id="c4e35-576">__Note.__</span></span> <span data-ttu-id="c4e35-577">`Out` 不是保留字。</span><span class="sxs-lookup"><span data-stu-id="c4e35-577">`Out` is not a reserved word.</span></span>

<span data-ttu-id="c4e35-578">使用 In 修饰符声明的类型参数是*逆变*的。</span><span class="sxs-lookup"><span data-stu-id="c4e35-578">A type parameter that is declared with the In modifier is *contravariant*.</span></span> <span data-ttu-id="c4e35-579">非正式类型参数（非正式类型参数）只能在输入位置使用--即传入接口或委托类型的值，不能用于输出位置。</span><span class="sxs-lookup"><span data-stu-id="c4e35-579">Informally, a contravariant type parameter can only be used in an input position -- i.e. a value that is being passed in to the interface or delegate type -- and cannot be used in an output position.</span></span> <span data-ttu-id="c4e35-580">如果以下情况，类型 `T` 被视为*有效的 contravariantly* ：</span><span class="sxs-lookup"><span data-stu-id="c4e35-580">A type `T` is considered to be *valid contravariantly* if:</span></span>

* <span data-ttu-id="c4e35-581">@no__t 为类、结构或枚举类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-581">`T` is a class, structure, or enumerated type.</span></span>

* <span data-ttu-id="c4e35-582">@no__t 为非泛型委托或接口类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-582">`T` is a non-generic delegate or interface type.</span></span>

* <span data-ttu-id="c4e35-583">@no__t 为数组类型，其元素类型为 contravariantly。</span><span class="sxs-lookup"><span data-stu-id="c4e35-583">`T` is an array type whose element type is valid contravariantly.</span></span>

* <span data-ttu-id="c4e35-584">`T` 是未在类型参数中声明为的类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-584">`T` is a type parameter which was not declared as an In type parameter.</span></span>

* <span data-ttu-id="c4e35-585">@no__t 为构造接口或委托类型，`X(Of P1,...,Pn)` 的类型参数 `A1,...,An`，以便：</span><span class="sxs-lookup"><span data-stu-id="c4e35-585">`T` is a constructed interface or delegate type `X(Of P1,...,Pn)` with type arguments `A1,...,An` such that:</span></span>

  * <span data-ttu-id="c4e35-586">如果 `Pi` 声明为 @no__t 类型参数，则 `Ai` 是有效的 contravariantly。</span><span class="sxs-lookup"><span data-stu-id="c4e35-586">If `Pi` was declared as an `Out` type parameter then `Ai` is valid contravariantly.</span></span>

  * <span data-ttu-id="c4e35-587">如果 `Pi` 声明为 @no__t 类型参数，则 `Ai` 是有效的 covariantly。</span><span class="sxs-lookup"><span data-stu-id="c4e35-587">If `Pi` was declared as an `In` type parameter then `Ai` is valid covariantly.</span></span>

<span data-ttu-id="c4e35-588">以下内容必须是接口或委托类型中的有效 contravariantly：</span><span class="sxs-lookup"><span data-stu-id="c4e35-588">The following must be valid contravariantly in an interface or delegate type:</span></span>

* <span data-ttu-id="c4e35-589">参数的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-589">The type of a parameter.</span></span>

* <span data-ttu-id="c4e35-590">方法类型参数上的类型约束。</span><span class="sxs-lookup"><span data-stu-id="c4e35-590">A type constraint on a method type parameter.</span></span>

* <span data-ttu-id="c4e35-591">如果属性具有 @no__t 的取值函数，则为属性的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-591">The type of a property if it has a `Set` accessor.</span></span>

* <span data-ttu-id="c4e35-592">事件的类型。</span><span class="sxs-lookup"><span data-stu-id="c4e35-592">The type of an event.</span></span>

<span data-ttu-id="c4e35-593">例如：</span><span class="sxs-lookup"><span data-stu-id="c4e35-593">For example:</span></span>
...
<span data-ttu-id="c4e35-594">在类型必须有效的情况下为 contravariantly 和 covariantly （例如，具有 @no__t 0 和 @no__t 访问器的属性或 `ByRef` 参数）时，不能使用变体类型参数。</span><span class="sxs-lookup"><span data-stu-id="c4e35-594">In the case where a type must be valid be contravariantly and covariantly (such as a property with both a `Get` and `Set` accessor or a `ByRef` parameter), a variant type parameter cannot be used.</span></span>


<span data-ttu-id="c4e35-595">协方差和方差会给出 "钻石多义性问题"。</span><span class="sxs-lookup"><span data-stu-id="c4e35-595">Co- and contra-variance give rise to a "diamond ambiguity problem".</span></span> <span data-ttu-id="c4e35-596">考虑下列代码：</span><span class="sxs-lookup"><span data-stu-id="c4e35-596">Consider the following code:</span></span>
...
<span data-ttu-id="c4e35-597">可以通过两种方式将类 `C` 转换为 `IEnumerable(Of Object)`，两者均通过从 `IEnumerable(Of String)` 到协变转换，以及从 `IEnumerable(Of Exception)` 到协变转换。</span><span class="sxs-lookup"><span data-stu-id="c4e35-597">The class `C` can be converted to `IEnumerable(Of Object)` in two ways, both through covariant conversion from `IEnumerable(Of String)` and through covariant conversion from `IEnumerable(Of Exception)`.</span></span> <span data-ttu-id="c4e35-598">CLR 未指定 @no__t 将调用这两种方法中的哪一种。</span><span class="sxs-lookup"><span data-stu-id="c4e35-598">The CLR does not specify which of the two methods will be called by `c.GetEnumerator()`.</span></span> <span data-ttu-id="c4e35-599">通常，每当将类声明为实现具有公共超类型的两个不同泛型参数的协变接口时（例如，在此情况下 `String`，`Exception` 具有公共父类型 `Object`），或者声明一个类来实现具有两个具有相同子类型的泛型参数的逆变接口，则可能出现多义性。</span><span class="sxs-lookup"><span data-stu-id="c4e35-599">In general, whenever a class is declared to implement a covariant interface with two different generic arguments that have a common supertype (e.g. in this case `String` and `Exception` have the common supertype `Object`), or a class is declared to implement a contravariant interface with two different generic arguments that have a common subtype, then ambiguity is likely to arise.</span></span> <span data-ttu-id="c4e35-600">编译器会对此类声明发出警告。</span><span class="sxs-lookup"><span data-stu-id="c4e35-600">The compiler gives a warning on such declarations.</span></span>
